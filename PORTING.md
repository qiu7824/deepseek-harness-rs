...
> 目标行为基线：`dsh-v0.1.5-rc.2`；Rust 开发版本：`0.1.3-alpha.26`。

## 当前产品边界

当前开发版本为 `0.1.3-alpha.26`；可下载版本以已通过验收的 Release 附件为准。Rust Host 提供有界双向历史窗口、引导输入、产物与垃圾槽、侧栏扩展、原生终端和受控浏览器。运行时接口、存储格式和视觉效果按 Rust 产品契约维护。
