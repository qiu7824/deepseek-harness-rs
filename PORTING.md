...
> 目标行为基线：`dsh-v0.1.7-alpha.2`；Rust 开发版本：`0.1.3-alpha.34+local.20260923.3`。

## 当前产品边界

当前开发版本为 `0.1.3-alpha.34+local.20260923.3`；可下载版本以已通过验收的 Release 附件为准。Rust Host 提供有界双向历史窗口、引导输入、产物与垃圾槽、侧栏扩展、原生终端和受控浏览器。运行时接口、存储格式和视觉效果按 Rust 产品契约维护。

Session 新写入使用 V4，旧记录按相邻版本迁移规则读取并保留原件，稳定事件坐标与 `seedLength` 兼容边界保留；投影缓存使用逐记录 v5。官方 V0/V1/V2/V3/V4 的显式导入及附件恢复已接入，无法保持事件次序的记录拒绝迁移。支持的协议、入口及限制以 README 和协议矩阵为准。
