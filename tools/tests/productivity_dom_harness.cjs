...
(async()=>{
 ...
 await act(()=>root.render(React.createElement(plugin.test.Menus,{scope,t})));assert.equal(document.querySelectorAll('input[type=checkbox]').length,5);
 ...
 const tasks=[...document.querySelectorAll('label')].find(e=>e.textContent==='Project tasks');await act(()=>tasks.querySelector('input').click());assert.deepEqual(changes.at(-1),['tasks',false]);
 await act(()=>root.unmount());assert.equal(listeners.size,0);dom.window.close();console.log('PASS productivity UI: task creation/completion, conflict-preserved drafts, shared revision writes, five menu toggles and cleanup');
