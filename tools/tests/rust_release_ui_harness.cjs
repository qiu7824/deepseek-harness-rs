...
assert.equal(api.imageCardModel('read', result).images[0], attachment, 'all tools retain returned image evidence');
assert.equal(api.imageCardModel('computer_use', { ...result, isError: true }).images[0], attachment, 'a failed result may still contain an authorized diagnostic image');
assert.equal(api.imageCardModel('computer_use', { ...result, content: [...result.content, { type: 'image', attachment }] }).images.length, 1, 'duplicate references do not duplicate image consumers');
...
assert.equal(api.GenericToolCard({ ...common, toolName: 'computer_use', block: { ...result, isError: true } }).props.state, 'error', 'rendering image evidence never changes the tool outcome');
