...
class ProductSurfaceContractTests(unittest.TestCase):
    ...
    def test_release_workflow_gates_and_verifies_every_portable_variant(self):
        workflow = (ROOT / ".github" / "workflows" / "release.yml").read_text(encoding="utf-8")
        verifier = (ROOT / "tools" / "verify_release_package.py").read_text(encoding="utf-8")
        manager = (ROOT / "release" / "windows" / "DshServiceManager.ps1").read_text(encoding="utf-8")
        ...
        self.assertIn("--variant no-skin", workflow)
        self.assertIn('prefix + "NO_SKIN"', verifier)
        self.assertIn("DshServiceManager.ps1", verifier)
        self.assertIn("NotifyIcon", manager)
        ...
        self.assertIn("MessagesFile: \"{#ChineseMessages}\"", installer)
        self.assertIn("/DChineseMessages=$language", workflow)
        self.assertTrue((ROOT / "packaging" / "windows" / "ChineseSimplified.isl").is_file())
        self.assertIn("ContextMenuStrip", manager)
        package = (ROOT / "tools" / "package_release.py").read_text(encoding="utf-8")
        self.assertIn("shutil.rmtree(stage/'web'/'dist'/'skins'", package)
        self.assertIn("no-skin archive leaks bundled skin assets", verifier)
        self.assertIn("full archive is missing bundled skin assets", verifier)
