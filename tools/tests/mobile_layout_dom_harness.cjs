...
async function run(modules, output) {
  ...
  const shell = () => h(layout.AppFrame, { useStore: selector => selector(state), usePanelInfo: selector => selector({activePanelId:null}), useSessions: selector => selector(sessions), actions, renderSlot: (name, props) => name === "main" ? composer : name === "sidebar" ? h("nav", { "data-test-sidebar-width": props.width }, h("button", { onClick: () => actions.toggleSidebar() }, "菜单")) : name === "details" ? h("section", null, h("h2", null, "上下文"), h("button", { onClick: () => actions.closeDetails() }, "关闭")) : null });
