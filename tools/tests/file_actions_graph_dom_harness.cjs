...
let exported,paused=false;const graphRequests=[],opened=[],timers=new Map(),slots=new Map();
...
const graphContext={ResizeObserver:class {observe(){} disconnect(){}},React,document,AbortController,URLSearchParams,Date,console,setTimeout:(fn,delay)=>{const id=setTimeout(()=>{timers.delete(id);fn()},delay);timers.set(id,delay);return id},clearTimeout:id=>{timers.delete(id);clearTimeout(id)},window:{__ModuleLoader__:{load:def=>{exported=def.factory(id=>id==='react'?React:jsx)}}},__DSH_FILE_ACTIONS__:{open:options=>opened.push(options)},fetch:async(url,options={})=>{graphRequests.push({url,options});if(url.endsWith('code-graph-cancel')){paused=true;return{ok:true,json:async()=>({})}}if(new URL(url,'http://fixture.invalid').searchParams.get('resume'))paused=false;return{ok:true,text:async()=>JSON.stringify({...graph,status:paused?'cancelled':graph.status}),json:async()=>({text:'source preview'})}}};
vm.runInNewContext(fs.readFileSync(path.join(plugins,'ui-code-graph.js'),'utf8'),graphContext);
exported.apply({slots:{inject:(_name,fn)=>fn(),register:(options,component)=>slots.set(options.id,{options,component})},inject:()=>{}});
assert.equal(slots.get('code-graph').options.name,'conversation.view');assert.equal(slots.get('code-graph').options.order,20);
...
await React.act(async()=>root.render(React.createElement(slots.get('code-graph').component,{sessionId:'graph-session'})));await wait();
assert.ok(graphRequests.length>0,'opening graph requests index automatically');assert.equal(document.querySelectorAll('.cg-node').length,2);assert.equal(timers.size,0,'ready graph does not poll');
const press=async label=>{await React.act(async()=>{const node=[...document.querySelectorAll('button')].find(node=>node.textContent===label||node.getAttribute('aria-label')===label);assert.ok(node,label);assert.equal(node.disabled,false,label);node.click()});await wait()};
await press('符号调用');await React.act(async()=>[...document.querySelectorAll('.cg-node')].find(n=>n.getAttribute('aria-label').startsWith('caller，')).click());await wait();
assert.match(document.querySelector('.cg-inspector').textContent,/src\/调用.rs:10/);
await press('聚焦关系');assert.ok(graphRequests.some(row=>new URL(row.url,'http://fixture.invalid').searchParams.get('selected')==='caller'));
assert.equal(document.querySelectorAll('.cg-node').length,2);
await React.act(async()=>document.querySelector('.cg-node[data-selected=true]').dispatchEvent(new dom.window.MouseEvent('dblclick',{bubbles:true})));
assert.equal(opened[0].path,'src/调用.rs');assert.equal(opened[0].line,10);
await press('打开源码');assert.equal(opened.at(-1).line,10);
const before=document.querySelector('output').textContent;await press('放大画布');assert.notEqual(document.querySelector('output').textContent,before);
await press('适应画布');assert.equal(document.querySelector('output').textContent,before);
assert.ok(![...document.querySelectorAll('button')].some(node=>node.textContent==='暂停'),'completed index cannot be cancelled');
graph.status='indexing';await press('更新');assert.ok([...timers.values()].includes(1400),'pending index schedules status refresh');
await press('暂停');assert.equal(JSON.parse(graphRequests.find(row=>row.url.endsWith('code-graph-cancel')).options.body).sessionId,'graph-session');assert.equal(timers.size,0,'cancelled index stops polling');
graph.status='ready';await press('更新');assert.ok(graphRequests.some(row=>new URL(row.url,'http://fixture.invalid').searchParams.get('resume')==='1'));
graph.status='indexing';await press('更新');assert.ok([...timers.values()].includes(1400));const finalRequest=graphRequests.at(-1);
await React.act(async()=>root.unmount());assert.equal(timers.size,0,'graph polling cleanup');assert.equal(finalRequest.options.signal.aborted,true,'unmount aborts the active graph effect');
console.log('PASS React DOM canvas: automatic index, file/symbol switch, focus query, exact source line, zoom/fit, pause/resume and cleanup');
}
