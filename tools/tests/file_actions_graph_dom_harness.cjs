...
await React.act(async()=>root.render(React.createElement(slots.get('code-graph').component,{sessionId:'graph-session'})));await wait();assert.ok(graphRequests.length>0,'opening graph requests index automatically');assert.equal(document.querySelectorAll('.dshGraphCard').length,2);assert.equal(timers.size,0,'ready graph does not poll');
...
const press=async label=>{await React.act(async()=>{const node=[...document.querySelectorAll('button')].find(node=>node.textContent===label);assert.ok(node,label);assert.equal(node.disabled,false,label);node.click()});await wait()};
assert.equal([...document.querySelectorAll('button')].find(node=>node.textContent==='取消索引').disabled,true,'completed index cannot be cancelled');
graph.status='indexing';await press('更新');assert.ok([...timers.values()].includes(1200),'pending index schedules status refresh');
await press('取消索引');assert.equal(JSON.parse(graphRequests.find(row=>row.url.endsWith('code-graph-cancel')).options.body).sessionId,'graph-session');assert.match(document.body.textContent,/自动索引已暂停/);assert.equal(timers.size,0,'cancelled index stops polling');
graph.status='ready';await press('继续索引');assert.ok(graphRequests.some(row=>new URL(row.url,'http://fixture.invalid').searchParams.get('resume')==='1'));
await press('读取符号');await press('读取 src/调用.rs:10–15');assert.equal(opened.at(-1).path,'src/调用.rs');
graph.status='indexing';await press('更新');assert.ok([...timers.values()].includes(1200));const finalRequest=graphRequests.at(-1);
