...
//! `@deepseek-ai/dsh-message-feedback`. Rust port of
//! `packages/feedback/message-feedback/src/invariant.ts`.
//!
//! No runtime invariant: durable row shapes are validated at the storage
//! domain boundary by the owning spec's schemas, and the service retains no
//! other observable relation.
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-message-feedback";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "message-feedback-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the message-feedback invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct MessageFeedbackInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for MessageFeedbackInvariantPlugin {
