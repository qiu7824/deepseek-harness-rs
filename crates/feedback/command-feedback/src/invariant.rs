...
//! `@deepseek-ai/dsh-command-feedback`. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-command-feedback";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "command-feedback-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the command-feedback invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct CommandFeedbackInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for CommandFeedbackInvariantPlugin {
