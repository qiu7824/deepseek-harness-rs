...
mod tests {
    ...
    #[test]
    fn id_pattern_boundary() {
        assert!(preset_id_ok("a"));
        assert!(preset_id_ok("standard"));
        assert!(preset_id_ok("a1-b2"));
        assert!(preset_id_ok("9lives"));
        assert!(!preset_id_ok(""));
        assert!(!preset_id_ok("-lead"));
        assert!(!preset_id_ok("UPPER"));
        assert!(!preset_id_ok("under_score"));
        assert!(!preset_id_ok("a..b"));
        assert!(!preset_id_ok("a/b"));
        assert!(!preset_id_ok("a b"));
        assert!(!preset_id_ok("中"));
    }

    #[test]
    fn unknown_preset_error_renders() {
        let error = UnknownPresetError::new("x", &["a".to_string(), "b".to_string()]);
        assert_eq!(
            error.to_string(),
            "agent-presets: preset \"x\" not found (available: a, b)"
        );
        let empty = UnknownPresetError::new("x", &[]);
        assert_eq!(
            empty.to_string(),
    ...
    #[test]
    fn mount_error_renders() {
        let error = PresetMountError::new("x", "broken row");
        assert_eq!(
            error.to_string(),
            "agent-presets: preset \"x\" failed to mount: broken row"
        );
    }
}
