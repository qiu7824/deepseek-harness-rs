//! Package-owned invariant companion for `@deepseek-ai/dsh-spill-local`: a
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-spill-local";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "spill-local-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the spill-local invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SpillLocalInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SpillLocalInvariantPlugin {
