//! Package-owned invariant companion for `@deepseek-ai/dsh-spill-policy`: a
//! no-op registration — no independent event sequence or mutable data
//! relation beyond contracts enforced at its owning seam (TS
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-spill-policy";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "spill-policy-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the spill-policy invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SpillPolicyInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SpillPolicyInvariantPlugin {
