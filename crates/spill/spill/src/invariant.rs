//! Package-owned invariant companion for `@deepseek-ai/dsh-spill`: a no-op
//! registration — this package exposes no independent event sequence or
//! mutable data relation beyond contracts enforced at its owning seam (TS
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-spill";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "spill-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the spill invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SpillInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SpillInvariantPlugin {
