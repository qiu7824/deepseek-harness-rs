//! Package-owned invariant companion for the subprocess seam: a no-op
//! registration — this stateless Service Definition owns spawn-spec/handle
//! types, while Service Providers own observations.
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-subprocess";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "subprocess-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the subprocess invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SubprocessInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SubprocessInvariantPlugin {
