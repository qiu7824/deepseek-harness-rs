...
//! `@deepseek-ai/dsh-repeat-tool-reminder`. Rust port of
//! `packages/guard/repeat-tool-reminder/src/invariant.ts`.
//!
//! No runtime invariant: the repeat chain is private to one post-execute
//! listener and exposes no package-owned event or snapshot that an
//! independent companion can observe (the TS install is a no-op).
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-repeat-tool-reminder";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "repeat-tool-reminder-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the repeat-tool-reminder invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct RepeatToolReminderInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for RepeatToolReminderInvariantPlugin {
