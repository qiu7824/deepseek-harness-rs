...
mod tests {
    ...
    fn config(root: &Path, compression: JsonlCompression) -> JsonlConfig {
        JsonlConfig {
    ...
    fn header(id: &str, created_at: u64, cwd: Option<&str>) -> SessionHeader {
        SessionHeader {
    ...
    async fn full_lifecycle_zstd() {
        ...
        // Lazy creation: no file until the first append.
        backend.create(header("s1", 1, None)).await.unwrap();
        assert!(backend.find_log(&session_id("s1")).await.unwrap().is_none());

        // Contiguity enforcement before any bytes hit the disk.
        let error = backend
            .append(&session_id("s1"), &[turn_event(7, 7)])
            .await
            .unwrap_err();
        ...
        backend
            ...
            .await
            .unwrap();
        let path = backend.find_log(&session_id("s1")).await.unwrap().unwrap();
        assert!(path.exists());
        assert!(path.to_string_lossy().ends_with("session.jsonl.zstd"));
        ...
        let inspection2 = backend.load(&session_id("s1")).await.unwrap();
        assert_eq!(inspection2.events, inspection.events);

        // List + snapshots see the metadata without a full-log parse.
        let listed = SessionPersistenceApi::list(backend.as_ref()).await.unwrap();
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].id.as_str(), "s1");
        let snapshots = backend.list_snapshots().await.unwrap();
        assert_eq!(snapshots.len(), 1);
        assert_eq!(snapshots[0].header.id.as_str(), "s1");

        // readRaw returns the exact artifact text with the header parsed.
        let raw = backend
            .read_raw(&session_id("s1"))
            .await
            .unwrap()
            .expect("artifact");
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn plaintext_mode_round_trips() {
        ...
        backend
            ...
            .await
            .unwrap();
        let path = backend.find_log(&session_id("p1")).await.unwrap().unwrap();
        assert!(path.to_string_lossy().ends_with("session.jsonl"));
        let bytes = std::fs::read(&path).unwrap();
        let text = String::from_utf8_lossy(&bytes);
        assert!(text.starts_with("{\"type\":\"session\""), "{text}");
        let inspection = backend.load(&session_id("p1")).await.unwrap();
    ...
    async fn torn_tail_is_truncated_and_repaired() {
        ...
        backend
            ...
            .await
            .unwrap();
        let path = backend.find_log(&session_id("t1")).await.unwrap().unwrap();
        // Simulate a crash tail: cut the final frame's checksum + tail.
        let bytes = std::fs::read(&path).unwrap();
        ...
        // the synthetic closer were re-appended as one fresh batch: two
        // complete frames (header + repaired batch), no structural torn
        // start.
        let after = std::fs::read(&path).unwrap();
        let scan = scan_zstd_frames(&after).unwrap();
        assert_eq!(scan.torn_start, None);
        assert_eq!(scan.frames.len(), 2);
        // A second load is idempotent.
        let inspection2 = backend.load(&session_id("t1")).await.unwrap();
        assert_eq!(inspection2.events, inspection.events);
        let _ = std::fs::remove_dir_all(&root);
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn live_session_write_path() {
        ...
        session
            ...
            .unwrap();
        // session/flush via the store drains through the backend.
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn id_and_encoding_collisions_reject() {
    ...
    #[test]
    fn config_parse_defaults_and_root_requirement() {
        let parsed = parse_config(&serde_json::json!({"root": "x"})).unwrap();
        assert!(parsed.pack_chunks);
        assert_eq!(parsed.compression, JsonlCompression::Zstd);
        ...
        .unwrap();
        assert!(!parsed.pack_chunks);
        assert_eq!(parsed.compression, JsonlCompression::None);
        assert_eq!(parsed.prepared_session_cache_size, 2);
        assert_eq!(parsed.write_batch_max_delay_ms, 50);

        assert!(parse_config(&serde_json::json!({})).is_err());
        assert!(parse_config(&serde_json::json!({"root": "x", "compression": "lz4"})).is_err());
    }
}
