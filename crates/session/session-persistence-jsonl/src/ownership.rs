...
impl JsonlSessionPersistence {
    pub(super) async fn check_writer_available(&self, id: &SessionId) -> Result<(), String> {
        ...
        let path = self.root.join(WRITER_DIRECTORY).join(crate::format::encode_segment(id.as_str())?);
        tokio::task::spawn_blocking(move || SessionGenerationLease::acquire(&path).map(drop))
            .await.map_err(|e| e.to_string())?
    }

    pub(super) async fn writer_lease(&self, meta: &SessionHeader) -> Result<Arc<WriterOwnership>, String> {
        if self.closed.load(Ordering::Acquire) {
            return Err("Session persistence is closed; no writer was admitted".into());
        }
        ...
        let identity = self.root.join(WRITER_DIRECTORY).join(crate::format::encode_segment(meta.id.as_str())?);
        ...
        let lease = tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, String>(Arc::new(WriterOwnership { _identity: identity, session }))
        }).await.map_err(|e| e.to_string())??;
    ...
    pub(super) async fn assert_legacy_authority(&self, directory: &Path, id: &SessionId) -> Result<(), String> {
        let directory = directory.to_owned(); let id = id.clone(); let compression = self.compression;
        tokio::task::spawn_blocking(move || check_legacy_authority(&directory, &id, compression))
            .await.map_err(|e| e.to_string())?
    }
}

fn check_legacy_authority(directory: &Path, id: &SessionId, compression: JsonlCompression) -> Result<(), String> {
    let Some(selected) = select_generation(directory, id.as_str())? else { return Ok(()) };
    if ![0, 3, 4].contains(&selected.version) {
        return Err(dsh_session_persistence::session_format_version_refusal(id, selected.version));
    }
    let _ = compression;
    Ok(())
}
