use cordis::Context;
use dsh_session::{
    CreateSessionMeta, CreateSessionOptions, Session, SessionHeader, SessionStore, session_id,
};
...
fn session(store: &SessionStore, id: &str, cwd: Option<String>) -> Session {
    ...
    session
        .append(
            "feedback/record",
            serde_json::json!({"text":"ownership fixture"}),
            None,
        )
        .unwrap();
    session
...
async fn unpublished_creation_excludes_same_id_across_projects_and_releases_on_drop() {
    let root = root();
    let (a_ctx, a_store, a) = backend(&root);
    let (b_ctx, b_store, b) = backend(&root);
    ...
    let second = session(
        &b_store,
        "shared-id",
        Some(root.join("other-project").to_string_lossy().into_owned()),
    );
    assert!(
        b.prepare_new(second.clone())
            .await
            .err()
            .unwrap()
    ...
    drop(first);
    let next = b.prepare_new(second).await.unwrap();
    drop(next);
    assert!(a.list().await.unwrap().is_empty());
    assert!(b.list().await.unwrap().is_empty());
    shutdown(&a_ctx).await;
    shutdown(&b_ctx).await;
...
async fn writer_ownership_survives_publication_and_readers_do_not_repair_the_live_log() {
    let root = root();
    let (a_ctx, a_store, a) = backend(&root);
    let (b_ctx, _, b) = backend(&root);
    let session = session(&a_store, "resident", None);
    let mut admission = a.prepare_new(session.clone()).await.unwrap();
    let detach = a_store.enter(&session).unwrap();
    a_store.announce(&session).await.unwrap();
    admission.dispose();
    session
        .append(
            "feedback/record",
            serde_json::json!({"text":"still owned"}),
            None,
        )
        .unwrap();
    ...
    let resumed = b.prepare(session.id()).await.unwrap();
    drop(resumed);
    shutdown(&a_ctx).await;
    shutdown(&b_ctx).await;
    std::fs::remove_dir_all(root).unwrap();
...
async fn abandoned_restore_and_committed_read_release_writer_ownership() {
    let root = root();
    let (seed_ctx, seed_store, seed) = backend(&root);
    let session = session(&seed_store, "cold", None);
    seed.create(session.header().clone(), None).await.unwrap();
    seed.append(session.id(), &session.events()).await.unwrap();
    shutdown(&seed_ctx).await;
    let (a_ctx, _, a) = backend(&root);
    let (b_ctx, _, b) = backend(&root);
    ...
    let after_read = a.prepare(session.id()).await.unwrap();
    drop(after_read);
    shutdown(&a_ctx).await;
    shutdown(&b_ctx).await;
    std::fs::remove_dir_all(root).unwrap();
...
async fn deleting_and_recreating_a_session_reuses_its_permanent_ownership_files() {
    let root = root();
    let (ctx, store, backend) = backend(&root);
    let session = session(&store, "delete-and-recreate", None);
    backend
        .create(session.header().clone(), None)
        .await
        .unwrap();
    backend
        ...
        .unwrap();
    let path = artifact(&root, session.header());
    assert!(backend.delete(session.id()).await.unwrap());
    assert!(!path.exists());
    assert!(
        path.parent()
            .unwrap()
            .join(".session-writer.lock")
            .is_file()
    );
...
async fn an_unknown_new_generation_refuses_cached_resume_and_old_artifact_reads() {
    let root = root();
    let (seed_ctx, store, seed) = backend(&root);
    let session = session(&store, "generation-authority", None);
    seed.create(session.header().clone(), None).await.unwrap();
    seed.append(session.id(), &session.events()).await.unwrap();
    shutdown(&seed_ctx).await;
    let (ctx, _, backend) = backend(&root);
    backend.inspect(session.id()).await.unwrap();
    let path = artifact(&root, session.header());
    let old = std::fs::read(&path).unwrap();
    let lease = dsh_session_persistence_jsonl::generations::SessionGenerationLease::acquire(
        path.parent().unwrap(),
    )
    .unwrap();
    let next = path.parent().unwrap().join("session.v5.jsonl");
    let mut future = serde_json::to_value(session.header()).unwrap();
    future["type"] = serde_json::json!("session");
...
async fn an_external_runtime_owner_blocks_writes_and_process_exit_releases_admission() {
    ...
    command
        .args([
            "--exact",
            "private_runtime_owner",
            "--ignored",
            "--nocapture",
        ])
        .env("DSH_WRITER_RUNTIME_ROOT", &root)
    ...
    while !ready.exists() && std::time::Instant::now() < deadline {
        assert!(
            ...
            "owner exited before acquiring runtime ownership"
        );
        tokio::time::sleep(std::time::Duration::from_millis(10)).await;
    }
    assert!(ready.exists());
    let (ctx, _, backend) = backend(&root);
    let id = session_id("external-owner");
    let inspected = backend.inspect(&id).await.unwrap();
    let path = artifact(&root, &inspected.meta);
    let before = std::fs::read(&path).unwrap();
    assert!(
        backend
            .prepare(&id)
    ...
    child.0.kill().unwrap();
    child.0.wait().unwrap();
    let acquired = backend.prepare(&id).await.unwrap();
    drop(acquired);
...
async fn private_runtime_owner() {
    let Some(root) = std::env::var_os("DSH_WRITER_RUNTIME_ROOT") else {
        return;
    };
    let root = PathBuf::from(root);
    let (_ctx, store, backend) = backend(&root);
