...
//! Rust port of `packages/session-query/session-query/src/invariant.ts`.
//!
//! No runtime invariant: query results are immutable per-call projections
//! whose lineage and event relations are validated while they are built; the
//! service retains no observable result state.
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-session-query";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "session-query-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the session-query invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SessionQueryInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SessionQueryInvariantPlugin {
