...
//! `@deepseek-ai/dsh-session-query-sqlite`. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-session-query-sqlite";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "session-query-sqlite-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the session-query-sqlite invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SqliteInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SqliteInvariantPlugin {
