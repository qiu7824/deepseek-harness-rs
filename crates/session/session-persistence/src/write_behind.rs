...
mod tests {
    ...
    fn counting_backend(
        writes: &Arc<parking_lot::Mutex<Vec<Vec<u64>>>>,
    ) -> Arc<dyn Fn(Vec<SessionEvent>) -> WriteFuture + Send + Sync> {
    ...
    async fn flush_drains_without_deadline_wait() {
        let writes = Arc::new(parking_lot::Mutex::new(Vec::<Vec<u64>>::new()));
        let controller = SessionWriteBehind::new(SessionWriteBehindOptions {
            max_delay_ms: 60_000,
            write: counting_backend(&writes),
            report_background_failure: Arc::new(|_| {}),
        });
        controller.enqueue(event(0));
        controller.enqueue(event(1));
        assert!(controller.has_work());
        // flush must NOT wait for the 60s deadline.
        controller.flush().await.unwrap();
        assert_eq!(&*writes.lock(), &[vec![0, 1]]);
        assert!(!controller.has_work());
        controller.flush().await.unwrap();
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn deadline_starts_a_background_write() {
        let writes = Arc::new(parking_lot::Mutex::new(Vec::<Vec<u64>>::new()));
        let controller = SessionWriteBehind::new(SessionWriteBehindOptions {
            max_delay_ms: 20,
            write: counting_backend(&writes),
        ...
        assert_eq!(&*writes.lock(), &[vec![0]]);
        assert!(!controller.has_work());
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn failure_retains_batch_and_reports_once() {
        let calls = Arc::new(AtomicU32::new(0));
        let failures = Arc::new(AtomicU32::new(0));
        let calls_for_write = calls.clone();
        let failures_for_report = failures.clone();
        let write: Arc<dyn Fn(Vec<SessionEvent>) -> WriteFuture + Send + Sync> =
            Arc::new(move |batch| {
                let calls = calls_for_write.clone();
                Box::pin(async move {
                    if calls.fetch_add(1, Ordering::SeqCst) == 0 {
                        return Err("disk full".to_string());
                    }
                    assert_eq!(batch.iter().map(|e| e.seq).collect::<Vec<_>>(), vec![0]);
                    Ok(())
                })
            });
        let controller = SessionWriteBehind::new(SessionWriteBehindOptions {
            max_delay_ms: 10,
            write,
            report_background_failure: Arc::new(move |_error| {
                failures_for_report.fetch_add(1, Ordering::SeqCst);
            }),
        });
        controller.enqueue(event(0));
        // The deadline write fails; the batch is retained and reported.
        tokio::time::sleep(std::time::Duration::from_millis(60)).await;
        assert_eq!(failures.load(Ordering::SeqCst), 1);
        assert!(controller.has_work(), "failed batch is retained");
        // A flush retries and succeeds.
        controller.flush().await.unwrap();
        assert!(!controller.has_work());
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn concurrent_flushes_join_one_barrier() {
        let writes = Arc::new(parking_lot::Mutex::new(Vec::<Vec<u64>>::new()));
        let controller = SessionWriteBehind::new(SessionWriteBehindOptions {
            max_delay_ms: 60_000,
            write: counting_backend(&writes),
        ...
        controller.enqueue(event(0));
        let (first, second) = tokio::join!(controller.flush(), controller.flush());
        first.unwrap();
        second.unwrap();
        assert_eq!(
            writes.lock().len(),
