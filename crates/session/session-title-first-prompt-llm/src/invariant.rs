...
//! `@deepseek-ai/dsh-session-title-first-prompt-llm`: a no-runtime-check
//! registration (the shared LLM policy validates and owns each auxiliary
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-session-title-first-prompt-llm";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "session-title-first-prompt-llm-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SessionTitleFirstPromptLlmInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SessionTitleFirstPromptLlmInvariantPlugin {
