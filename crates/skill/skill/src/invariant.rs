...
//! Rust port of `packages/skill/skill/src/invariant.ts`.
//!
//! No runtime invariant: provider/runtime maps and revisioned caches mutate
//! atomically inside the registry, which exposes no independent change
//! event or snapshot for cross-checking them.
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-skill";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "skill-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the skill invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SkillInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SkillInvariantPlugin {
