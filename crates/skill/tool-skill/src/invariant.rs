...
//! Rust port of `packages/skill/tool-skill/src/invariant.ts`.
//!
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-tool-skill";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "tool-skill-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the tool-skill invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct ToolSkillInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for ToolSkillInvariantPlugin {
