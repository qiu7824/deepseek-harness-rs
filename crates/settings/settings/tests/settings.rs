...
fn theme_schema() -> Schema {
    ...
    properties.insert("theme".to_string(), Schema::union(vec![
        Schema::constant(Data::String("dark".to_string())),
        Schema::constant(Data::String("light".to_string())),
    ])
    .default(Data::String("dark".to_string())));
    properties.insert("font_size".to_string(), Schema::number().default(Data::Number(14.0)));
    Schema::object(properties)
}

fn nested_schema() -> Schema {
    let mut retry = IndexMap::new();
    retry.insert("attempts".to_string(), Schema::number().default(Data::Number(2.0)));
    retry.insert("delayMs".to_string(), Schema::number().default(Data::Number(100.0)));
    let mut properties = IndexMap::new();
    properties.insert("retry".to_string(), Schema::object(retry));
    properties.insert("tags".to_string(), Schema::array(Schema::string()).default(Data::Array(vec![Data::String("default".to_string())])));
    Schema::object(properties)
}
...
impl SettingsStorage for MemorySettings {
    ...
    async fn persist(&self, ns: &SettingsNamespace, section: Data) -> Result<(), String> {
        self.persisted.lock().push((ns.as_str().to_string(), section.clone()));
...
async fn brands_lowercase_kebab_case_names() {
    ...
    for invalid in ["", "UI", "9lives", "a_b", "-lead"] {
        assert!(settings_namespace(invalid).is_err(), "{invalid} must reject");
    }
}
...
async fn resolves_defaults_then_base_then_user_layer() {
    ...
    doc.insert("ui-theme".to_string(), data(serde_json::json!({"theme": "light"})));
    let (ctx, provider, _) = boot(Some(doc)).await;
    let scope = provider
        .register(&ctx, ns("ui-theme"), theme_schema(), SettingsRegisterOptions {
    ...
    let value = (scope.get)();
    assert_eq!(data(serde_json::json!({"theme": "light", "font_size": 16})), value);
}
...
async fn validate_rejects_bad_write_and_publish_keeps_last_good() {
    ...
    let mut doc = IndexMap::new();
    doc.insert("ui-theme".to_string(), data(serde_json::json!({"font_size": 4})));
    provider.publish(doc, dsh_settings::SettingsUpdateSource::Provider);
    assert_eq!((scope.get)(), before);

    provider.update(&ns("ui-theme"), serde_json::json!({"font_size": 18}), None).await.unwrap();
    let value = (scope.get)();
    assert_eq!(data(serde_json::json!({"theme": "dark", "font_size": 18})), value);
}
...
async fn rejects_duplicate_and_invalid_sections() {
    let (ctx, provider, _) = boot(None).await;
    provider.register(&ctx, ns("ui-theme"), theme_schema(), SettingsRegisterOptions::default()).unwrap();
    let error = provider
        .register(&ctx, ns("ui-theme"), theme_schema(), SettingsRegisterOptions::default())
        .err()
    ...
    let error = provider2
        .register(&ctx2, ns("ui-other"), theme_schema(), SettingsRegisterOptions::default())
        .err()
    ...
    let mut doc = IndexMap::new();
    doc.insert("ui-other".to_string(), data(serde_json::json!({"font_size": "big"})));
    let (ctx3, provider3, _) = boot(Some(doc)).await;
    assert!(provider3
        .register(&ctx3, ns("ui-other"), theme_schema(), SettingsRegisterOptions::default())
        .is_err());
}
...
async fn update_persists_merged_section_without_baking_base() {
    ...
    let scope = provider
        ...
        .unwrap();
    (scope.update)(serde_json::json!({"theme": "dark"})).await.unwrap();
    let persisted = storage.persisted.lock().clone();
    assert_eq!(persisted.len(), 1);
    assert_eq!(persisted[0].1, data(serde_json::json!({"theme": "dark"})));
    assert_eq!((scope.get)(), data(serde_json::json!({"theme": "dark", "font_size": 16})));
}
...
async fn update_deep_merges_nested_objects_and_replaces_arrays() {
    ...
    let (ctx, provider, storage) = boot(Some(doc)).await;
    let scope = provider
        .register(&ctx, ns("workspace"), nested_schema(), SettingsRegisterOptions::default())
        .unwrap();
    (scope.update)(serde_json::json!({"retry": {"attempts": 7}, "tags": ["c"]})).await.unwrap();
    let persisted = storage.persisted.lock().clone();
    assert_eq!(
        persisted[0].1,
...
async fn commits_notifies_watchers_and_emits_update_event() {
    ...
    let seen_for_event = Arc::clone(&seen);
    {
        ...
        let listener: Arc<cordis::Listener> = Arc::new(move |_ctx, args: Vec<cordis::ArcValue>| {
            let ns = cordis::downcast::<SettingsNamespace>(&args[0]).cloned().unwrap();
        ...
        let _ = futures::executor::block_on(event_ctx.on("settings/updated", listener, cordis::EventOptions::default()));
        let scope = provider
            .register(&ctx, ns("ui-theme"), theme_schema(), SettingsRegisterOptions::default())
            .unwrap();
        (scope.watch)(Arc::new({
        ...
        (scope.update)(serde_json::json!({"theme": "light"})).await.unwrap();
        ...
        assert_eq!(events_for_assert.lock().clone(), vec!["ui-theme".to_string()]);
    }
}
...
async fn rejects_invalid_patch_before_persisting() {
    ...
    provider.register(&ctx, ns("ui-theme"), theme_schema(), SettingsRegisterOptions::default()).unwrap();
    assert!(provider.update(&ns("ui-theme"), serde_json::json!({"font_size": "big"}), None).await.is_err());
...
async fn replace_resets_and_expected_revision_rejects_stale() {
    ...
    let scope = provider
        ...
        .unwrap();
    (scope.update)(serde_json::json!({"theme": "light", "font_size": 20})).await.unwrap();
    ...
    let error = provider
        .update(&ns("ui-theme"), serde_json::json!({"font_size": 22}), Some(0))
        .await
        .err()
...
async fn mutate_applies_path_ops_on_the_current_section() {
    ...
    provider
        .mutate(
            &ns("workspace"),
            vec![
                SettingsPathOp::Set { path: vec!["retry".to_string(), "attempts".to_string()], value: serde_json::json!(9) },
                SettingsPathOp::Unset { path: vec!["retry".to_string(), "delayMs".to_string()] },
            ],
            None,
        )
        .await
        .unwrap();
    let persisted = storage.persisted.lock().clone();
    assert_eq!(persisted[0].1, data(serde_json::json!({"retry": {"attempts": 9}})));
    assert_eq!(
        (scope.get)(),
...
async fn publish_moves_revision_and_emits_document_updated() {
    ...
    let seen: Arc<Mutex<Vec<u64>>> = Arc::new(Mutex::new(Vec::new()));
    {
        ...
        let _ = futures::executor::block_on(
            ctx.on("settings/document-updated", listener, cordis::EventOptions::default()),
        );
    }
    let mut doc = IndexMap::new();
    doc.insert("ui-theme".to_string(), data(serde_json::json!({"theme": "light"})));
    ...
    assert_eq!(provider.describe(SettingsDescribeOptions::default())[0].revision, 1);
}
...
async fn describe_redacts_secret_fields() {
    ...
    let mut properties = IndexMap::new();
    properties.insert("endpoint".to_string(), Schema::string().default(Data::String("https://x".to_string())));
    ...
    provider
        ...
        .await
        .unwrap();
    let descriptors = provider.describe(SettingsDescribeOptions { redact_secrets: true });
    assert_eq!(descriptors.len(), 1);
    assert_eq!(descriptors[0].secrets, vec![dsh_settings::RedactedSecret {
    ...
    match value {
        Data::Object(object) => {
            ...
            assert_eq!(object.get("endpoint"), Some(&Data::String("https://x".to_string())));
        }
...
async fn deep_equal_json_compares_structurally() {
    assert!(deep_equal_json(&data(serde_json::json!({"a": [1, {"b": 2}]})), &data(serde_json::json!({"a": [1, {"b": 2}]}))));
    assert!(!deep_equal_json(&data(serde_json::json!({"a": 1})), &data(serde_json::json!({"a": 2}))));
    assert!(!deep_equal_json(&data(serde_json::json!([1, 2])), &data(serde_json::json!([1, 3]))));
}
...
async fn removes_namespace_when_registrant_fiber_disposes() {
    ...
    assert!(provider.describe(SettingsDescribeOptions::default()).is_empty());
}
...
async fn install_settings_section_wires_source_and_falls_back() {
    ...
    let current: Arc<Mutex<Option<Arc<dyn Fn() -> Data + Send + Sync>>>> = Arc::new(Mutex::new(None));
    ...
    assert_eq!(*changes.lock(), 1);
    {
        ...
        assert_eq!(source(), data(serde_json::json!({"theme": "dark", "font_size": 12})));
    }
    ...
    {
        let source = current.lock().clone().expect("source set");
        assert_eq!(source(), data(serde_json::json!({"theme": "light", "font_size": 12})));
    }
