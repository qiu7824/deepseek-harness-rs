...
impl NativeViewInputRuntime {
    ...
    pub(crate) fn window_close_request_command(&self) -> Option<&Command> {
        self.window_close_request_command.as_ref()
    }

    pub(crate) fn pending_app_commands(&self) -> &[Command] {
        &self.pending_app_commands
    }
...
impl NativeWindowBuilder {
    ...
    /// Attaches one platform-native status item to this single-window app.
    pub fn tray(mut self, tray: TraySpec) -> Self {
        self.tray = Some(tray);
        self
    }
    ...
    pub fn build(self) -> ZsuiResult<ZsuiApp> {
        ...
        let app = app(self.app_name).window(self.window);
        match self.tray {
            Some(tray) => app.tray(tray).build(),
            None => app.build(),
        }
    }
...
impl<ContentState> TypedNativeWindowBuilder<ContentState> {
    ...
    pub fn tray(mut self, tray: TraySpec) -> Self {
        self.inner = self.inner.tray(tray);
        self
    }
