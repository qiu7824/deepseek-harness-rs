...
impl WindowsWin32ViewInputRoute {
    ...
    fn dispatch_window_close_requested(&mut self) -> WindowsWin32ViewInputDispatchReport {
        ...
        let mut report = self.adapt_shared_report(report, WindowsSharedInputKind::WindowClose);
            ...
            .filter(|command| windows_win32_window_lifecycle_action(command))
        {
            ...
            report.quit_requested |= command == &Command::Quit;
            if report.window_lifecycle_commands.is_empty() {
                report.window_lifecycle_commands.push(command.clone());
            }
        }
        report
    }
    ...
    fn adapt_shared_report(
        ...
        report.window_lifecycle_commands.extend(
            self.shared_runtime
