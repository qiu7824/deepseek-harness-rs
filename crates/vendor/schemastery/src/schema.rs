...
mod tests {
    ...
    fn num(value: impl Into<f64>) -> Data {
        Data::Number(value.into())
    }
    ...
    fn scalars_validate() {
        assert_eq!(
            Schema::validate(&Schema::string(), s("hi")).unwrap(),
            s("hi")
        );
        assert_eq!(
            Schema::validate(&Schema::number(), num(1.5)).unwrap(),
            num(1.5)
        );
        assert_eq!(
            Schema::validate(&Schema::boolean(), Data::Bool(true)).unwrap(),
            Data::Bool(true)
        );
    ...
    fn required_and_defaults() {
        let schema = Schema::object(IndexMap::from([
            ("name".to_string(), Schema::string().required(true)),
            ("count".to_string(), Schema::number().default(num(3))),
        ]));
    ...
    #[test]
    fn number_step_constraints() {
        assert!(Schema::validate(&Schema::natural(), num(0.5)).is_err());
        assert!(Schema::validate(&Schema::natural(), num(3)).is_ok());
        // decimal step path (0.1): 0.3 - 0 == 3 * 0.1 → ok; 0.25 fails
        let step_schema = Schema::number().step(0.1);
        assert!(Schema::validate(&step_schema, num(0.3)).is_ok());
        assert!(Schema::validate(&step_schema, num(0.25)).is_err());
        assert!(Schema::validate(&Schema::number().min(5.0).max(10.0), num(11)).is_err());
    }

    #[test]
    fn string_pattern_and_length() {
        let schema = Schema::string().pattern("^a+$", None);
        assert!(Schema::validate(&schema, s("aaa")).is_ok());
        assert!(Schema::validate(&schema, s("baa")).is_err());
        let bounded = Schema::string().max(3.0);
        assert!(Schema::validate(&bounded, s("abcd")).is_err());
        assert!(Schema::validate(&bounded, s("abc")).is_ok());
    }
    ...
    fn bitset_normalizes() {
        ...
        assert!(Schema::validate(&schema, s("a")).is_err());
    }
    ...
    fn union_tries_in_order() {
        ...
        assert!(error.contains("expected number | string"), "got: {error}");
    }

    #[test]
    fn intersect_merges_objects() {
        let schema = Schema::intersect(vec![
            Schema::object(IndexMap::from([("a".to_string(), Schema::number())])),
            Schema::object(IndexMap::from([("b".to_string(), Schema::string())])),
        ]);
    ...
    #[test]
    fn transform_preserve_and_adaptation() {
        let double = Schema::transform(
            Schema::number(),
            Arc::new(|value: &Data, _options: &Options| {
                Ok(Data::Number(value.as_number().unwrap() * 2.0))
            }),
            true,
        );
        assert_eq!(Schema::validate(&double, num(21)).unwrap(), num(42));

        // non-preserve adaptation writes the converted value back into the
        // input object (TS `data[key] = adapted`)
        let schema = Schema::object(IndexMap::from([(
            "x".to_string(),
            Schema::transform(
                Schema::string(),
                Arc::new(|value: &Data, _options: &Options| match value {
                    Data::String(text) => Ok(num(text.parse().unwrap_or(0.0))),
                    _ => unreachable!("string schema"),
                }),
                false,
            ),
        )]));
        let value = Schema::validate(&schema, obj(&[("x", s("7"))])).unwrap();
        assert_eq!(value, obj(&[("x", num(7))]));
    }

    #[test]
    fn dict_renames_keys_via_skey() {
        let upper = Schema::transform(
            Schema::string(),
            Arc::new(|value: &Data, _options: &Options| match value {
                Data::String(text) => Ok(s(&text.to_uppercase())),
                _ => unreachable!("string schema"),
            }),
            true,
        );
        let schema = Schema::dict(Schema::number(), Some(upper));
        let value = Schema::validate(&schema, obj(&[("foo", num(1))])).unwrap();
        assert_eq!(value, obj(&[("FOO", num(1))]));
    }

    #[test]
    fn lazy_recursive_schema() {
        fn tree() -> Schema {
            Schema::object(IndexMap::from([
                ("name".to_string(), Schema::string()),
                (
                    "children".to_string(),
                    Schema::array(Schema::lazy(Arc::new(tree))),
                ),
            ]))
        }
        let input = obj(&[
            ...
            (
                ...
                Data::Array(vec![obj(&[
                    ("name", s("leaf")),
                    ("children", Data::Array(vec![])),
                ])]),
            ),
        ]);
        assert!(Schema::validate(&tree(), input).is_ok());
        let bad = obj(&[("name", s("root")), ("children", Data::Array(vec![num(1)]))]);
        assert!(Schema::validate(&tree(), bad).is_err());
    }

    #[test]
    fn autofix_removes_invalid_members() {
        let schema = Schema::object(IndexMap::from([
            ("a".to_string(), Schema::number()),
        ...
        assert_eq!(data, obj(&[("a", num(1))]));
    }

    #[test]
    fn loose_returns_default() {
        let schema = Schema::number().default(num(9)).loose(true);
        assert_eq!(Schema::validate(&schema, s("x")).unwrap(), num(9));
    }

    #[test]
    fn standard_schema_v1_results() {
        let schema = Schema::object(IndexMap::from([(
            "x".to_string(),
            Schema::number().required(true),
        )]));
        match schema.standard_validate(obj(&[("x", num(1))])) {
            StandardResult::Success { value } => assert_eq!(value, obj(&[("x", num(1))])),
            other => panic!("expected success, got {other:?}"),
        }
        match schema.standard_validate(obj(&[])) {
            StandardResult::Failure { issues } => {
                assert_eq!(issues.len(), 1);
                assert!(issues[0].message.contains("missing required value"));
                assert_eq!(issues[0].path, vec![PathSeg::Key("x".to_string())]);
            }
    ...
    #[test]
    fn from_infers_schemas() {
        assert_eq!(Schema::from(None).type_name(), "any");
        let constant = Schema::from(Some(&s("x")));
        assert_eq!(constant.type_name(), "const");
        assert!(constant.meta().required);
        assert!(Schema::validate(&constant, s("x")).is_ok());
        assert!(Schema::validate(&constant, s("y")).is_err());
    }

    #[test]
    fn simplify_collapses_defaults() {
        let schema = Schema::object(IndexMap::from([
            ("a".to_string(), Schema::number()),
            ("b".to_string(), Schema::string()),
        ]));
        // empty object equals the schema default → null
        assert_eq!(schema.simplify(&obj(&[])), Data::Null);
        let filled = obj(&[("a", num(1))]);
        assert_eq!(schema.simplify(&filled), filled);
    }

    #[test]
    fn date_and_regexp_nodes() {
        let date = Schema::date();
        assert!(Schema::validate(&date, s("2024-01-02T03:04:05Z")).is_ok());
        assert!(Schema::validate(&date, s("not-a-date")).is_err());
        let regexp = Schema::reg_exp("");
        let Data::RegExp { source, .. } = Schema::validate(&regexp, s("a+")).unwrap() else {
    ...
    #[test]
    fn array_buffer_nodes() {
        let schema = Schema::array_buffer(Some("base64"));
        let Data::Binary(bytes) = Schema::validate(&schema, s("TWE=")).unwrap() else {
            panic!("expected binary");
        };
        assert_eq!(bytes, b"Ma");
    }

    #[test]
    fn type_strings_match_ts_format() {
        assert_eq!(Schema::number().type_string(false), "number");
        assert_eq!(
            Schema::array(Schema::string()).type_string(false),
            "string[]"
        );
        assert_eq!(
            Schema::union(vec![Schema::number(), Schema::string()]).type_string(false),
            "number | string"
        );
        assert_eq!(
            Schema::object(IndexMap::from([(
