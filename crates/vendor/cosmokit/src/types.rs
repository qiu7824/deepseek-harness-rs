...
mod tests {
    ...
    fn base64_round_trip() {
        ...
        // lenient like Buffer.from
        assert_eq!(from_base64("TW\nFu"), b"Man");
        assert_eq!(from_base64("TW!F!u"), b"Man");
    }

    #[test]
    fn hex_round_trip() {
        assert_eq!(to_hex(b"\x00\xff\x10"), "00ff10");
        assert_eq!(from_hex("00ff10"), b"\x00\xff\x10");
        // lenient: odd length drops the trailing char
        assert_eq!(from_hex("abc"), from_hex("ab"));
    }

    #[test]
    fn equality_semantics() {
        assert!(deep_equal(&json!(1), &json!(1), false));
        assert!(!deep_equal(&json!(1), &json!(2), false));
        assert!(deep_equal(&json!(null), &json!(null), true));
        assert!(deep_equal(&json!(null), &json!(null), false));
