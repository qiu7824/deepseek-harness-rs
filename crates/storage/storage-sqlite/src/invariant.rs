...
//! a no-runtime-check registration (single-statement primitives and the
//! schema stamp/reject sequence are proven by the package spec).
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-storage-sqlite";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "storage-sqlite-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the storage-sqlite invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SqliteStorageInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SqliteStorageInvariantPlugin {
