...
mod tests {
    ...
    #[test]
    fn journal_modes_cover_the_ts_union() {
        assert_eq!(JournalMode::Wal.pragma(), "WAL");
        assert_eq!(JournalMode::Delete.pragma(), "DELETE");
        assert_eq!(JournalMode::Truncate.pragma(), "TRUNCATE");
        assert_eq!(JournalMode::Persist.pragma(), "PERSIST");
    }

    #[test]
    fn open_memory_database_stamps_and_creates_tables() {
        let db = open_database(":memory:", JournalMode::default()).expect("open");
        let version: i64 = db
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .expect("version");
    ...
    fn schema_version_mismatch_rejects() {
        let path = std::env::temp_dir().join(format!("dsh-sqlite-schema-{}", std::process::id()));
    ...
    /// Keep the row shape helper used by unit tests visible.
