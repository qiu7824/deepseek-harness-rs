...
mod tests {
    ...
    #[test]
    fn web_and_headless_project_the_code_runtime_surface_capability() {
        for (surface, rows) in [("web", web_rows()), ("headless", headless_rows())] {
            let ids: Vec<&str> = rows
                .iter()
                .filter_map(|row| row.get("id").and_then(Value::as_str))
                .collect();
            assert!(
                ids.contains(&"code-runtime"),
                "shipped {surface} bundle is missing code-runtime: {ids:?}"
            );
        }
    }

    #[test]
    fn base_bundle_projects_the_common_execution_stack() {
        let rows = base_rows();
        let ids: Vec<&str> = rows
            .iter()
            .filter_map(|row| row.get("id").and_then(Value::as_str))
        ...
        for required in [
            ...
            assert!(
                ...
                "shipped base bundle is missing {required}: {ids:?}"
            );
        }
    }
}
