...
//! `@deepseek-ai/dsh-tool-str-replace-editor`. Rust port of
//! `packages/fs/tool-str-replace-editor/src/invariant.ts`.
//!
//! No runtime invariant: the tool adapter owns no independent durable
//! state; filesystem mutation relations stay with the provider and policy
//! plugins.
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-tool-str-replace-editor";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "tool-str-replace-editor-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the tool-str-replace-editor invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct ToolStrReplaceEditorInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for ToolStrReplaceEditorInvariantPlugin {
