...
async fn registers_the_standalone_schema_and_configurable_description() {
    ...
    assert_eq!(
        ...
        Some(ToolCallView::Generic {
            ...
            raw_input: None,
            content: None,
            locations: Some(vec![dsh_tools::FileLocation {
    ...
    let create = present(
        &serde_json::json!({ "command": "create", "path": "/workspace/a.txt", "file_text": "hello" }),
    );
    assert_eq!(
        create,
    ...
    let replace = present(
        &serde_json::json!({ "command": "str_replace", "path": "/workspace/a.txt", "old_str": "old", "new_str": "new" }),
    );
    assert_eq!(
        replace,
        Some(ToolCallView::Diff {
    ...
    let insert = present(
        &serde_json::json!({ "command": "insert", "path": "/workspace/a.txt", "insert_line": 0, "new_str": "x" }),
    );
    assert_eq!(
        insert,
        Some(ToolCallView::Generic {
    ...
    let create_empty =
        present(&serde_json::json!({ "command": "create", "path": "/workspace/empty.txt" }));
    match create_empty {
    ...
    let replace_empty =
        present(&serde_json::json!({ "command": "str_replace", "path": "/workspace/a.txt" }));
    match replace_empty {
        Some(ToolCallView::Diff { diffs, .. }) => {
    ...
    let insert_bare =
        present(&serde_json::json!({ "command": "insert", "path": "/workspace/a.txt" }));
    match insert_bare {
        Some(ToolCallView::Generic { locations, .. }) => {
            assert_eq!(
                locations,
                Some(vec![dsh_tools::FileLocation {
                    path: "/workspace/a.txt".to_string(),
                    line: None
                }])
            );
...
async fn lists_visible_entries_to_depth_two_and_clips_at_the_configured_view_limit() {
    ...
    let world = setup_with(
        root.clone(),
        SetupOptions {
            config: serde_json::json!({ "maxOutputChars": 10_000 }),
            ..Default::default()
        },
    )
    .await;
    ...
    let listing = text(
        call(
            &world,
            owner,
            serde_json::json!({ "command": "view", "path": root.join("dir").to_string_lossy() }),
        )
        .await
        .as_ref(),
    );
    ...
    let clipped = setup_with(
        clipped_root.clone(),
        SetupOptions {
            config: serde_json::json!({ "maxOutputChars": 10 }),
            ..Default::default()
        },
    )
    .await;
    ...
    .await;
    assert!(
        text(&view).contains("<response clipped>"),
        "{}",
        text(&view)
    );
    let _ = std::fs::remove_dir_all(&root);
...
async fn matches_canonical_empty_line_range_and_end_insert_behavior() {
    ...
    let empty_view = call(
        &world,
        owner.clone(),
        serde_json::json!({ "command": "view", "path": root.join("empty.txt").to_string_lossy() }),
    )
    .await;
    assert!(
    ...
    let ownerless_view = call(
        &world,
        None,
        serde_json::json!({ "command": "view", "path": root.join("plain.txt").to_string_lossy() }),
    )
    .await;
...
async fn passes_the_session_sandbox_policy_to_every_mutation() {
    ...
    assert!(
        text(&result).contains("[sandbox: file access denied under read-only mode]"),
        "{}",
        text(&result)
    );
...
async fn preserves_tabs_outside_the_edited_region() {
    ...
    let view = call(
        &world,
        owner.clone(),
        serde_json::json!({ "command": "view", "path": root.join("Makefile").to_string_lossy() }),
    )
    .await;
...
async fn reports_missing_sandbox_policy_composition_during_plugin_startup() {
    ...
    *mock.confined.lock() = Some(SandboxMode::ReadOnly);
    ...
    assert!(
        error
            .message()
            .contains("the mounted filesystem confines but ctx.sandboxPolicy is missing"),
        "{error}"
    );
    let _ = std::fs::remove_dir_all(&root);
}
...
async fn maps_unexpected_backend_write_failures_for_replace_and_insert() {
    ...
    *world.mock.write_override.lock() = Some(Arc::new(
        |_target, _content, _expected, _signal, _policy| {
            Box::pin(async move {
    ...
    assert!(
        text(&replace).contains("backend write failed"),
        "{}",
        text(&replace)
    );
    ...
    assert!(
        text(&insert).contains("backend write failed"),
        "{}",
        text(&insert)
    );
    let _ = std::fs::remove_dir_all(&root);
