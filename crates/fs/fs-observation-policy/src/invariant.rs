...
//! `@deepseek-ai/dsh-fs-observation-policy`: a no-op registration — this
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-fs-observation-policy";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "fs-observation-policy-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the fs-observation-policy invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct FsObservationPolicyInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for FsObservationPolicyInvariantPlugin {
