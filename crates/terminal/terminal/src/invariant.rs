//! Package-owned invariant companion for the terminal seam: a no-op
//! registration — backend and owner-scoped session registries are private
//! mutable state, and the service exposes neither an independent lifecycle
//! stream nor an unscoped snapshot. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-terminal";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "terminal-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the terminal invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct TerminalInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for TerminalInvariantPlugin {
