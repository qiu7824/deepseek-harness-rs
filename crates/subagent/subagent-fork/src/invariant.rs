...
//! `@deepseek-ai/dsh-subagent-fork-in-process`. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-subagent-fork-in-process";

/// Cordis companion plugin name.
pub const NAME: &str = "subagent-fork-in-process-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the fork invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct ForkInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for ForkInvariantPlugin {
