...
//! `@deepseek-ai/dsh-subagent-spawn-in-process`. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-subagent-spawn-in-process";

/// Cordis companion plugin name.
pub const NAME: &str = "subagent-spawn-in-process-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the spawn invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct SpawnInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for SpawnInvariantPlugin {
