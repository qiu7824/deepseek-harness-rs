    ...
    pub fn agent_options(&self) -> dsh_agent::AgentOptions {
        dsh_agent::AgentOptions {
            ...
            reasoning_effort: (!self.reasoning_effort.is_empty())
                .then(|| dsh_llm::reasoning_effort_id(&self.reasoning_effort)),
...
impl Config {
    pub fn validate(&self) -> Result<(), String> {
        if !(1..=16).contains(&self.max_members)
            || !matches!(self.default_mode.as_str(), "off" | "auto" | "custom")
        ...
        for profile in &self.profiles {
            if !super::name(&profile.id)
                || !ids.insert(&profile.id)
                || profile.name.trim().is_empty()
                || profile.name.len() > 128
                || profile.roles.is_empty()
                || profile.roles.len() > 16
            {
                return Err("invalid or duplicate collaboration profile".into());
            }
            let mut roles = BTreeSet::new();
            for role in &profile.roles {
                role.validate()?;
                if !roles.insert(&role.id) {
                    return Err("duplicate collaboration role".into());
                }
            }
        }
        if !self.default_profile.is_empty() && !ids.contains(&self.default_profile) {
            return Err("default collaboration profile does not exist".into());
        }
        if self.default_mode == "custom" && self.default_profile.is_empty() {
            return Err("custom collaboration requires a default profile".into());
        }
        Ok(())
    }
    pub fn parse(mut value: serde_json::Value) -> Result<Self, String> {
        ...
        fn integer(value: &mut serde_json::Value, maximum: u64) -> Result<(), String> {
            ...
            let number = value
                .as_f64()
                .filter(|n| n.is_finite() && n.fract() == 0.0 && *n >= 1.0 && *n <= maximum as f64)
                .ok_or("collaboration limits must be positive integers within their bounds")?;
            *value = serde_json::json!(number as u64);
            Ok(())
        }
        if let Some(limit) = value.get_mut("maxMembers") {
            integer(limit, 16)?;
        }
        if let Some(profiles) = value.get_mut("profiles").and_then(|v| v.as_array_mut()) {
            for profile in profiles {
                if let Some(roles) = profile.get_mut("roles").and_then(|v| v.as_array_mut()) {
                    for role in roles {
                        if let Some(limit) = role.get_mut("maxTokens") {
...
fn explicit_configuration() -> bool {
    true
}
...
mod tests {
    ...
    fn legacy_settings_and_role_routes_are_validated() {
        ...
        assert!(old.enabled);
        assert_eq!(old.default_mode, "off");
        assert_eq!(
            Config::parse(serde_json::json!({"maxMembers":8.0}))
                .unwrap()
                .max_members,
            8
        );
        ...
        role.model = "model-a".into();
        role.max_tokens = Some(2048);
        role.validate().unwrap();
        let options = role.agent_options();
        assert_eq!(options.provider.as_deref(), Some("route"));
