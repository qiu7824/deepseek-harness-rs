...
async fn idle_deadline_survives_cancellation_polling_and_heartbeats_do_not_count_as_progress() {
    for heartbeat in [false, true] {
        ...
        let ctx = Context::root();
        let runtime = LlmRuntime::install(&ctx);
        apply(&ctx, &runtime, adapter).unwrap();
        let chunks: Vec<_> =
            tokio::time::timeout(Duration::from_secs(2), runtime.stream(options()).collect())
                .await
                .expect("stream deadlines must settle without a browser refresh");
        let failures = chunks
            .iter()
            .filter_map(|chunk| {
        ...
        assert_eq!(failures[0].code, "TIMEOUT");
        if heartbeat {
            assert!(failures[0].message.contains("stream_progress"));
        }
        server.abort();
    }
}
