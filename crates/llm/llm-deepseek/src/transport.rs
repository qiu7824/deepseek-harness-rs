...
mod tests {
    ...
    async fn missing_response_headers_have_a_deadline_and_close_the_socket() {
        ...
        let server = tokio::spawn(async move {
            ...
            tokio::time::timeout(std::time::Duration::from_secs(1), socket.read(&mut bytes))
                .await
                .expect("cancelled HTTP connection must close")
                .unwrap()
        });
        let result = super::post_with_deadline(
            &format!("http://{address}"),
            None,
            b"{}".to_vec(),
            &[],
    ...
    async fn a_hung_error_body_is_cancellable() {
        ...
        let response = post(
            ...
            b"{}".to_vec(),
            &[],
            Some(Arc::new(move || flag.load(Ordering::SeqCst))),
        )
        .await
        .unwrap();
        ...
        let result = tokio::time::timeout(
            ...
            response.collect_limited(1024),
        )
        .await
        .unwrap();
        assert!(result.err().unwrap().contains("phase:error_body"));
        server.await.unwrap();
    }
    #[tokio::test]
