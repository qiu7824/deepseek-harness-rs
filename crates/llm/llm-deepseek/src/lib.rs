...
mod image_offload_tests {
    ...
    fn options(count: usize, bytes: u64) -> GenerateOptions {
        GenerateOptions {
            provider: PROVIDER.to_string(),
            model: "vision".to_string(),
            reasoning_effort: None,
            messages: vec![dsh_llm::create_user_message(
                (0..count).map(|index| image(index, bytes)).collect(),
    ...
    fn image_ids(options: &GenerateOptions) -> Vec<String> {
        request_image_attachments(options)
            .into_iter()
    ...
    #[test]
    fn estimate_caps_master_lengths_and_offloads_one_stable_prefix() {
        let durable = options(129, 9 * 1024 * 1024);
        let durable_ids = image_ids(&durable);
        let projected = project_estimated_request(&durable);
        assert_eq!(image_ids(&projected), durable_ids[65..]);
        assert_eq!(image_ids(&durable), durable_ids);
    }

    #[test]
    fn file_and_inline_exact_stages_start_from_the_same_estimated_keep_set() {
        let durable = options(601, 1);
        let estimated = project_estimated_request(&durable);
        let keep = image_ids(&estimated);
        assert_eq!(keep.len(), 581);
        let meta: std::collections::HashMap<_, _> = keep
            .iter()
            .map(|id| {
                (
                    id.clone(),
        ...
        let files =
            project_exact_request(&estimated, &meta, dsh_llm::RequestImageRepresentation::Raw);
        let inline = project_exact_request(
            &estimated,
            &meta,
            dsh_llm::RequestImageRepresentation::Base64,
        );
        assert_eq!(image_ids(&files), keep);
        assert_eq!(image_ids(&inline), keep);
    }
}
