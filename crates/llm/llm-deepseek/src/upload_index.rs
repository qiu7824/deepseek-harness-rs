...
struct CleanupRecord {record:DeepSeekUploadRecord,attempts:u32,next_attempt_at:u64}
...
impl DeepSeekUploadIndex {
        ...
        dsh_atomic_write::with_file_lock(&path, async {
            ...
            if let Some(existing) = index.records.iter().find(|record| {
                ...
                if existing.file_id!=candidate.file_id {
                    index.pending_cleanup.push(CleanupRecord{record:candidate,attempts:0,next_attempt_at:now});
                    self.save(&index).await.map_err(std::io::Error::other)?;
                }
            ...
            index.pending_cleanup.extend(evicted.iter().filter(|record|record.file_id!=candidate.file_id).cloned().map(|record|CleanupRecord{next_attempt_at:record.expires_at,record,attempts:0}));
            self.save(&index).await.map_err(std::io::Error::other)?;
    ...
    pub async fn pending_cleanup(&self,scope:&DeepSeekFileScope,now:u64)->Result<Vec<DeepSeekUploadRecord>,String>{
        let index=self.load().await?;
        Ok(index.pending_cleanup.iter().filter(|entry|entry.record.scope==*scope&&entry.next_attempt_at<=now&&!index.records.iter().any(|record|record.scope==*scope&&record.file_id==entry.record.file_id)).take(32).map(|entry|entry.record.clone()).collect())
    }
    pub async fn cleanup_result(&self,scope:&DeepSeekFileScope,id:&DeepSeekFileId,success:bool,now:u64)->Result<(),String>{
        dsh_atomic_write::with_file_lock(&self.path,async {
            let mut index=self.load().await.map_err(std::io::Error::other)?;
            if success {index.pending_cleanup.retain(|entry|!(entry.record.scope==*scope&&entry.record.file_id==*id));}
            else {for entry in &mut index.pending_cleanup {if entry.record.scope==*scope&&entry.record.file_id==*id {entry.attempts=entry.attempts.saturating_add(1);entry.next_attempt_at=now.saturating_add((1000u64<<entry.attempts.min(12)).min(3_600_000));}}}
            self.save(&index).await.map_err(std::io::Error::other)
...
mod cleanup_tests {
    ...
    async fn cleanup_is_durable_scoped_and_backed_off_after_failure() {
        ...
        let record=|id:&str|DeepSeekUploadRecord{scope:scope.clone(),attachment_id:dsh_attachment::attachment_id("sha256:fixture"),variant_id:dsh_attachment::image_variant_id("sha256:variant"),file_id:deepseek_file_id(id),bytes:1,created_at:0,expires_at:1000};
        index.commit(record("first"),0,0).await.unwrap();
        let duplicate=index.commit(record("unused"),0,0).await.unwrap();assert!(!duplicate.accepted);
        assert_eq!(index.pending_cleanup(&scope,0).await.unwrap().len(),1);
        index.cleanup_result(&scope,&deepseek_file_id("unused"),false,0).await.unwrap();
        assert!(index.pending_cleanup(&scope,0).await.unwrap().is_empty());
        assert_eq!(index.pending_cleanup(&scope,3000).await.unwrap().len(),1);
        assert!(index.pending_cleanup(&deepseek_file_scope("https://fixture.invalid","other-account"),3000).await.unwrap().is_empty());
        index.cleanup_result(&scope,&deepseek_file_id("unused"),true,3000).await.unwrap();
        assert!(index.pending_cleanup(&scope,4000).await.unwrap().is_empty());
        assert!(directory.starts_with(std::env::temp_dir()));std::fs::remove_dir_all(directory).unwrap();
    }
}
