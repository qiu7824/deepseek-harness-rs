//! Package-owned invariant companion for `dsh-native-command`: a no-op
//! registration — this library is stateless (no ctx, no events); the runner
//! contract is exercised through its own tests. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-native-command";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "native-command-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the native-command invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct NativeCommandInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for NativeCommandInvariantPlugin {
