...
//! `@deepseek-ai/dsh-launch-environment`: a no-op registration — the
//! snapshot is frozen before any fiber starts and this package owns no event
//! stream or mutable runtime data; its lookup and rejection rules are
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-launch-environment";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "launch-environment-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the launch-environment invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct LaunchEnvironmentInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for LaunchEnvironmentInvariantPlugin {
