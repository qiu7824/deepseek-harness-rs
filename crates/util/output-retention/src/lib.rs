...
mod tests {
    ...
    fn item_retainer_head() {
        ...
        assert_eq!(result.omitted, Omitted::Exact { count: 1 });
        assert!(result.truncated);
    }

    #[test]
    fn text_retainer_head_trims_utf8() {
        // "héllo" = h(1) é(2) l(1) l(1) o(1); cap 2 lands mid-é.
        let mut retainer = TextRetainer::new(TextRetentionStrategy::Head { max_bytes: 2 });
        let decision = retainer.push("héllo".as_bytes());
        assert!(!decision.kept);
        let result = retainer.finish();
        // trailing partial é trimmed: only "h" survives the cut;
        // 6 total bytes − 1 kept = 5 omitted
        assert_eq!(result.text, "h");
        assert_eq!(result.omitted_bytes, Omitted::Exact { count: 5 });
    }

    #[test]
    fn text_retainer_head_complete_cut_keeps_boundary() {
        // cap 5 on "héllo" lands on a complete boundary: "héll" kept.
        let mut retainer = TextRetainer::new(TextRetentionStrategy::Head { max_bytes: 5 });
        retainer.push("héllo".as_bytes());
        let result = retainer.finish();
        assert_eq!(result.text, "héll");
        assert_eq!(result.omitted_bytes, Omitted::Exact { count: 1 });
    }

    #[test]
    fn text_retainer_head_exact_boundary() {
        let mut retainer = TextRetainer::new(TextRetentionStrategy::Head { max_bytes: 3 });
        retainer.push("hé".as_bytes()); // h + é = 3 bytes, complete
        let result = retainer.finish();
        assert_eq!(result.text, "hé");
        assert_eq!(result.omitted_bytes, Omitted::None);
    }

    #[test]
    fn text_retainer_tail() {
        let mut retainer = TextRetainer::new(TextRetentionStrategy::Tail { max_bytes: 3 });
        retainer.push("abcdef".as_bytes());
        let result = retainer.finish();
        assert_eq!(result.text, "def");
        assert_eq!(result.omitted_bytes, Omitted::Exact { count: 3 });
    }
    ...
    fn text_retainer_head_tail_skips_middle() {
        ...
        assert_eq!(result.omitted_bytes, Omitted::Exact { count: 4 });
    }

    #[test]
    fn notice_wording() {
        assert_eq!(describe_omitted(Omitted::None, NoticeUnit::Items), "");
        assert_eq!(
            describe_omitted(Omitted::Exact { count: 3 }, NoticeUnit::Bytes),
            "Omitted 3 bytes."
        );
        assert_eq!(
            describe_omitted(Omitted::Unknown, NoticeUnit::Lines),
            "More lines were omitted."
        );
        let notice = RetentionNotice {
            scope: "grep".to_string(),
            strategy: NoticeStrategy::Head,
            unit: NoticeUnit::Items,
            limit: NoticeLimit::Single(20),
            kept: 20,
            omitted: Omitted::Exact { count: 5 },
        };
        let footer = format_retention_notice(&notice, |_| "Narrow the pattern.".to_string());
        assert_eq!(footer, "Omitted 5 items. Narrow the pattern.");
    }
}
