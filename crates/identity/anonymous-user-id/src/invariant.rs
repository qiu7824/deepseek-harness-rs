//! Package-owned invariant companion for
//! `@deepseek-ai/dsh-anonymous-user-id`. Rust port of
//! `packages/identity/anonymous-user-id/src/invariant.ts`.
//!
//! No runtime invariant: the API owns one private memo and one best-effort
//! file, with no independent event stream or public mutable relation for a
//! companion to compare without creating the identity as a side effect (the
//! TS install is a no-op).

use std::sync::Arc;

use cordis::{ArcValue, Context, InjectSpec, Plugin, PluginError};
use dsh_invariants::{InvariantInstaller, InvariantRegistry};

/// Full package name reserved with the invariant registry.
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-anonymous-user-id";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "anonymous-user-id-invariant";

/// Service required before the companion can reserve package ownership.
pub const INJECT: [&str; 1] = ["invariants"];

/// Build the no-op installer registered under [`PACKAGE_NAME`].
pub fn installer() -> InvariantInstaller {
    InvariantInstaller {
        inject: None,
        install: Arc::new(|_ctx: &Context, _fail: Arc<dyn Fn(&str) + Send + Sync>| {
            Box::pin(async {})
        }),
    }
}

/// Register this package's invariant companion (TS `apply`).
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        .get_typed::<Arc<InvariantRegistry>>("invariants", false)
        .expect("the anonymous-user-id invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct AnonymousUserIdInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for AnonymousUserIdInvariantPlugin {
    fn name(&self) -> Option<&'static str> {
        Some(NAME)
    }

    fn inject(&self) -> InjectSpec {
        InjectSpec::new(INJECT)
    }

    async fn apply(&self, ctx: &Context, _config: ArcValue) -> Result<(), PluginError> {
        apply(ctx);
        Ok(())
    }
}
