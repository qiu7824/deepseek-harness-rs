...
fn fails_deletion_while_the_registry_still_publishes_the_entity() {
    ...
    assert!(
        failures[0].contains("cache still publishes"),
        "{}",
        failures[0]
    );
}
...
async fn installer_listens_to_domain_changes_against_the_live_registry_cache() {
    ...
    let dir = futures::executor::block_on(dsh_workspace::realpath_normalize(&temp.dir("wired")))
        .expect("canonical");
    ...
    result
        .ctx
        .emit("domain/changed", vec![cordis::arc(put_change(&key))]);
    ...
    result
        .ctx
        .emit("domain/changed", vec![cordis::arc(deleted_change(&key))]);
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    {
        let failures = failures.lock();
        assert_eq!(failures.len(), 1);
    ...
    result
        .ctx
        .emit("domain/changed", vec![cordis::arc(put_change("missing"))]);
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    let failures = failures.lock();
...
async fn companion_plugin_registers_with_the_invariants_service() {
    let temp = TempRoot::new();
    let dir = futures::executor::block_on(dsh_workspace::realpath_normalize(&temp.dir("smoke")))
        .expect("canonical");
    let result = harness(Arc::new(MemoryMediaPool::new()), &[], None).await;
    ...
    result.ctx.emit(
        "domain/changed",
        vec![cordis::arc(put_change(&workspace.id().to_string()))],
    );
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    (disposer)().await;
}
