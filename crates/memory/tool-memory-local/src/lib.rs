...
impl Default for MemoryDocument {
    fn default() -> Self {
        Self { version: 1, revision: 0, entries: Vec::new() }
    }
}
...
impl cordis::Service for MemoryStore {
    fn service_name(&self) -> &'static str { "memoryStore" }
}

impl MemoryStore {
    pub async fn open(root: PathBuf) -> Result<Self, String> {
        tokio::fs::create_dir_all(&root).await.map_err(|e| format!("memory directory: {e}"))?;
        let path = root.join("entries.json");
        let document = match tokio::fs::read(&path).await {
            Ok(bytes) => serde_json::from_slice(&bytes).map_err(|e| format!("memory document: {e}"))?,
        ...
        Ok(Self { root, document: tokio::sync::Mutex::new(document) })
    }

    pub async fn list(&self, scope: Option<&str>, category: Option<&str>) -> Vec<MemoryEntry> {
        let document = self.document.lock().await;
        document.entries.iter().filter(|entry| {
            scope.is_none_or(|value| entry.scope == value)
                && category.is_none_or(|value| entry.category == value)
        }).cloned().collect()
    }

    pub async fn upsert(&self, mut entry: MemoryEntry, expected_revision: Option<u64>) -> Result<MemoryEntry, String> {
        validate_entry(&entry)?;
        let mut document = self.document.lock().await;
        if let Some(position) = document.entries.iter().position(|candidate| candidate.id == entry.id) {
            let actual = document.entries[position].revision;
            if expected_revision.is_some_and(|expected| expected != actual) {
                return Err(format!("memory entry changed since it was read (expected revision {}, now {actual})", expected_revision.unwrap()));
            }
            entry.revision = actual + 1;
            document.entries[position] = entry.clone();
        } else {
            if entry.id.trim().is_empty() { entry.id = uuid::Uuid::new_v4().to_string(); }
            if expected_revision.is_some() { return Err("memory entry does not exist".to_string()); }
            entry.revision = 1;
    ...
    pub async fn remove(&self, id: &str, expected_revision: Option<u64>) -> Result<bool, String> {
        let mut document = self.document.lock().await;
        let Some(position) = document.entries.iter().position(|entry| entry.id == id) else { return Ok(false); };
        let actual = document.entries[position].revision;
        if expected_revision.is_some_and(|expected| expected != actual) {
            return Err(format!("memory entry changed since it was read (expected revision {}, now {actual})", expected_revision.unwrap()));
        }
    ...
    pub async fn render_enabled(&self, scope: &str, budget: usize) -> String {
        ...
        for entry in document.entries.iter().filter(|entry| entry.enabled && (entry.scope == "default" || entry.scope == scope)) {
            let label = BUILTIN_CATEGORIES.iter().find(|(id, _)| *id == entry.category).map(|(_, label)| *label).unwrap_or(&entry.category);
            let next = format!("\n- [{label}] {}: {}", entry.title, entry.content);
            if rendered.chars().count() + next.chars().count() > budget { break; }
...
fn validate_entry(entry: &MemoryEntry) -> Result<(), String> {
    if entry.scope.trim().is_empty() { return Err("memory scope must be non-empty".to_string()); }
    if !BUILTIN_CATEGORIES.iter().any(|(id, _)| *id == entry.category) { return Err("unknown memory category".to_string()); }
    if entry.title.trim().is_empty() { return Err("memory title must be non-empty".to_string()); }
    if entry.content.trim().is_empty() { return Err("memory content must be non-empty".to_string()); }
    Ok(())
}
...
pub fn render_enabled_file(root: &Path, scope: &str, budget: usize) -> String {
    let Ok(bytes) = std::fs::read(root.join("entries.json")) else { return String::new(); };
    let Ok(document) = serde_json::from_slice::<MemoryDocument>(&bytes) else { return String::new(); };
    let mut rendered = String::new();
    for entry in document.entries.iter().filter(|entry| entry.enabled && (entry.scope == "default" || entry.scope == scope)) {
        let label = BUILTIN_CATEGORIES.iter().find(|(id, _)| *id == entry.category).map(|(_, label)| *label).unwrap_or(&entry.category);
        let next = format!("\n- [{label}] {}: {}", entry.title, entry.content);
    ...
    if rendered.is_empty() { String::new() } else { format!("Agent 长期记忆（用户可在设置中编辑）：{rendered}") }
}
...
pub fn install(ctx: &Context, root: PathBuf) -> Result<(), String> {
    ...
    let store = Arc::new(futures::executor::block_on(MemoryStore::open(root.clone()))?);
...
mod tests {
    ...
    fn entry(id: &str, category: &str, content: &str) -> MemoryEntry {
        MemoryEntry {
            id: id.to_string(), scope: "default".to_string(), category: category.to_string(),
            title: "测试记忆".to_string(), content: content.to_string(), enabled: true, revision: 0,
        }
    }
    ...
    async fn categorized_entries_survive_reopen_and_filter_by_scope() {
        ...
        store.upsert(entry("one", "known-error", "不要重复旧错误"), None).await.unwrap();
        ...
        assert_eq!(reopened.list(None, Some("known-error")).await[0].content, "不要重复旧错误");
    ...
    async fn stale_edits_are_rejected_and_disabled_entries_are_not_rendered() {
        let root = std::env::temp_dir().join(format!("dsh-memory-{}", uuid::Uuid::new_v4()));
        let store = MemoryStore::open(root.clone()).await.unwrap();
        let saved = store.upsert(entry("one", "user-preference", "使用中文"), None).await.unwrap();
        let mut changed = saved.clone();
        changed.content = "使用简体中文".to_string();
        let changed = store.upsert(changed, Some(saved.revision)).await.unwrap();
        assert!(store.upsert(saved, Some(1)).await.unwrap_err().contains("now 2"));
