...
mod tests {
    ...
    #[test]
    fn reservation_errors_are_distinct_from_retryable_invalidity() {
        assert_eq!(
            reservation_verdict(Err("projection failed".to_string())),
            ReservationVerdict::InfrastructureFailure
        );
        assert_eq!(reservation_verdict(Ok(false)), ReservationVerdict::Invalid);
    }

    fn test_attempt(label: &str) -> RoundAttempt {
        let message = create_user_message(
            vec![ContentBlock::Text {
    ...
    #[test]
    fn reservation_slot_never_overwrites_an_existing_attempt() {
        let slot = Mutex::new(Some(test_attempt("existing")));
        let replacement = test_attempt("replacement");

        assert!(!reserve_attempt(&slot, replacement));
        assert_eq!(
            slot.lock().as_ref().expect("existing attempt").content,
            vec![ContentBlock::Text {
    ...
    #[tokio::test]
    async fn task_tracker_closes_registration_before_drain_observes_zero() {
        let tracker = Arc::new(DriverTaskTracker::new());
        let live = tracker.try_register().expect("first task registers");
        let tracker_for_stop = tracker.clone();
        let stopping = tokio::spawn(async move {
            tracker_for_stop.stop_and_drain().await;
        });
        tokio::task::yield_now().await;

        assert!(
            tracker.try_register().is_none(),
            "registration closes before drain"
        );
        assert!(!stopping.is_finished(), "drain must wait for the live task");
        drop(live);
    ...
    #[tokio::test]
    async fn task_guard_notification_survives_until_the_waiter_is_polled() {
        let tracker = Arc::new(DriverTaskTracker::new());
        let guard = tracker.try_register().expect("task registers");
        let changed = tracker.changed.notified();

        drop(guard);

        assert_eq!(tracker.state.lock().active, 0);
        tokio::time::timeout(std::time::Duration::from_millis(20), changed)
            .await
            .expect("a task exit before the waiter polls must leave a notification permit");
    }
}
