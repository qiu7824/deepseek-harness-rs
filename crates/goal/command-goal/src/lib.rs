...
mod tests {
    use super::*;
    use dsh_goal::{GoalBlockReason, goal_id};

    fn view(phase: GoalPhase, blocked_reason: Option<GoalBlockReason>) -> GoalView {
        GoalView {
            id: goal_id("renderer-test"),
            revision: 1,
            objective: "ship safely".to_string(),
            phase,
            blocked_reason,
            max_goal_rounds: 4,
            rounds_started: 1,
    ...
    #[test]
    fn blocked_rendering_fails_closed_without_a_reason() {
        let outcome =
            std::panic::catch_unwind(|| render_goal("Goal", &view(GoalPhase::Blocked, None)));
        assert!(
            outcome.is_err(),
            "a blocked goal without its reason must fail closed"
        );
    }

    #[test]
    fn non_blocked_rendering_ignores_a_stale_reason() {
        let rendered = render_goal(
            ...
            &view(
                ...
                Some(GoalBlockReason {
                    ...
                    message: "must not render".to_string(),
                }),
            ),
        );
        let CommandResult::Success {
            ...
            panic!("renderer succeeds");
        };
        assert!(!rendered.contains("Blocker:"), "{rendered}");
    }
}
