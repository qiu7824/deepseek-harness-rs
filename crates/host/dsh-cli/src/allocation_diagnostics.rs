...
fn bucket(size: usize) -> usize { (usize::BITS - size.max(1).leading_zeros()).min((BINS - 1) as u32) as usize }
fn allocated(size: usize) {
    let size = size as u64;
    let live = LIVE.fetch_add(size, Ordering::Relaxed) + size;
    PEAK.fetch_max(live, Ordering::Relaxed);
    let index = bucket(size as usize);
    COUNTS[index].fetch_add(1, Ordering::Relaxed); BYTES[index].fetch_add(size, Ordering::Relaxed);
}
fn released(size: usize) {
    LIVE.fetch_sub(size as u64, Ordering::Relaxed);
    let index = bucket(size);
    COUNTS[index].fetch_sub(1, Ordering::Relaxed); BYTES[index].fetch_sub(size as u64, Ordering::Relaxed);
}
...
unsafe impl GlobalAlloc for MeasuredAllocator {
    ...
    unsafe fn dealloc(&self, pointer: *mut u8, layout: Layout) {
        released(layout.size()); unsafe { mimalloc::MiMalloc.dealloc(pointer, layout) };
    }
    unsafe fn realloc(&self, pointer: *mut u8, layout: Layout, size: usize) -> *mut u8 {
        let resized = unsafe { mimalloc::MiMalloc.realloc(pointer, layout, size) };
        if !resized.is_null() { released(layout.size()); allocated(size); }
        resized
    }
}
pub fn initialize() {
    REPORT.get_or_init(|| std::env::var_os("DSH_ALLOCATION_REPORT").map(|path| {
    ...
    unsafe { std::env::remove_var("DSH_ALLOCATION_REPORT"); }
}
pub fn sample() {
    let Some(Some(output)) = REPORT.get() else { return; };
    let live = LIVE.load(Ordering::Relaxed); let peak = PEAK.load(Ordering::Relaxed);
    ...
    let time = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis();
    ...
    if let Ok(mut output) = output.lock() { let _ = writeln!(output,"{row}"); let _ = output.flush(); }
}
