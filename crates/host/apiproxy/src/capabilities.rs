...
pub struct CapabilityManager {
    ...
    lifecycle: Arc<crate::skill_lifecycle::SkillLifecycle>,
}
...
impl CapabilityManager {
        ...
        ctx.effect(
            ...
            Box::pin(async move {
                Some(cordis::make_disposer(move || {
                    ...
                    let lifecycle_provider = lifecycle_provider.clone();
                    Box::pin(async move {
    ...
    pub async fn invoke(&self, method: &str, payload: Value) -> Result<Value, String> {
        if method.starts_with("capabilities.skillRevision") {
            return self.revision_invoke(method, payload).await;
        }
    ...
    pub fn skill_lifecycle(&self) -> Arc<crate::skill_lifecycle::SkillLifecycle> {
    ...
    async fn revision_invoke(&self, method: &str, payload: Value) -> Result<Value, String> {
        ...
        match method {
            "capabilities.skillRevisionToggle" => {
                ...
                self.lifecycle.set_enabled(expected, enabled).await?;
