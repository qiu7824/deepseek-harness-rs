...
use dsh_remote_execution::{Connection, RemoteRuntime};
        ...
        handler: Arc::new(move |request| {
            ...
            Box::pin(async move {
                    ...
                    .await
                    {
                        Ok(bytes) => match serde_json::from_slice::<Value>(&bytes) {
                            Ok(args) => match args["action"].as_str().unwrap_or("list") {
                                "list" => Ok(remote.list()),
                                "connect" => match serde_json::from_value::<Connection>(
                                    args["connection"].clone(),
                                ) {
                                    Ok(connection) => {
                                        remote.connect(connection).await.and_then(|row| {
                                ...
                                "remove" => remote
                                    .remove(args["id"].as_str().unwrap_or_default())
                                    .await
                                ...
                                "query" | "cancel" => {
                                    remote
                                        .query(
                                            ...
                                            args["executionId"].as_str().unwrap_or_default(),
                                            "read-only",
                                            args["stdoutOffset"].as_u64().unwrap_or(0),
                                            args["stderrOffset"].as_u64().unwrap_or(0),
                                            args["action"] == "cancel",
                                        )
                                        .await
                                }
                ...
                Ok(http::Response::builder()
                    ...
                    .expect("remote execution response"))
            })
        }),
    })
}
...
mod tests {
    ...
    async fn composed_host_routes_remote_paths_without_local_fallback() {
        ...
        assert!(
            ctx.get_typed::<Arc<RemoteRuntime>>("remoteExecution", false)
                .is_some()
        );
        let fs = ctx
        ...
        let local = fs.resolve(&root.to_string_lossy(), None).await.unwrap();
        assert!(fs.stat(&local, None).await.unwrap().is_some());
