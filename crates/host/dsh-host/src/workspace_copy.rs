...
pub fn promote_validated_tracked(
    ...
    if !expected.is_null()
        && !expected.as_str().is_some_and(|value| {
            value.len() == 64
                && value
                    .bytes()
...
mod tests {
    ...
    fn rejected_promotion_has_no_effect_boundary_and_success_marks_before_publication() {
        ...
        let mut candidate = store
            .allocate("owner", &project.to_string_lossy(), "candidate", "result")
            .unwrap();
        let id = candidate.id().to_owned();
        let source = candidate.path().join("result.txt");
        std::fs::write(&source, b"validated output").unwrap();
        ...
        let refused: Arc<dyn Fn() -> Result<(), String> + Send + Sync> =
            Arc::new(|| Err("execution already ended".into()));
        assert!(
            promote_validated_tracked(
                &store,
                &id,
        ...
        assert!(seen.load(Ordering::SeqCst));
        assert_eq!(
            std::fs::read(project.join("result.txt")).unwrap(),
            b"validated output"
        );
        drop(candidate);
        drop(store);
