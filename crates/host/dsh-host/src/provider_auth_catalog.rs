    ...
    let official = native
        ...
        && reqwest::Url::parse(
            profile
                .get("baseURL")
                .and_then(Value::as_str)
                .unwrap_or(dsh_llm_deepseek::PUBLIC_BASE_URL),
        )
        .is_ok_and(|url| url.scheme() == "https" && url.host_str() == Some("api.deepseek.com"));
    let native_flash = official.then(|| {
        let defaults =
            dsh_llm_deepseek::resolve_adapter_options(&dsh_llm_deepseek::DeepSeekConfig::default())
        ...
        serde_json::to_value(
            defaults
                ...
                .expect("native Flash model"),
        )
        .expect("native model JSON")
    });
        ...
        if let Some(row) = rows.get_mut(id) {
            ...
            if id == "deepseek-flash" && official {
                for field in ["systemPromptUpdate", "imageInput"] {
                    if let Some(value) = model.get(field).filter(|value| !value.is_null()) {
...
mod tests {
    ...
    fn official_flash_upgrade_preserves_legacy_rows_and_explicit_preferences_only_on_its_route() {
        ...
        let flash = rows
            ...
            .unwrap();
        assert_eq!(flash["enabled"], false);
        assert_eq!(flash["name"], "Preferred Flash");
        assert_eq!(flash["contextWindow"], 128000);
        assert_eq!(flash["imageInput"], true);
        assert_eq!(flash["systemPromptUpdate"], "in-history");
        assert_eq!(
            rows.iter()
        ...
        for native in [false, true] {
            let mut third_party = profile.clone();
            third_party["baseURL"] = json!("https://gateway.example/v1");
            assert!(
                !merge_models(&third_party, &directory, native)
                    .iter()
                    .any(|row| row["id"] == "deepseek-flash")
            );
        }
        let remote = catalog("alice", json!([{"id":"deepseek-flash"}]));
        assert_eq!(
            merge_models(&profile, &remote, true)
                .iter()
                .find(|row| row["id"] == "deepseek-flash")
                .unwrap()["systemPromptUpdate"],
            "in-history"
        );
    }
    #[test]
