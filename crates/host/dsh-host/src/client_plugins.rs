...
mod tests {
    ...
    fn root(name: &str) -> PathBuf {
        let path =
            std::env::temp_dir().join(format!("dsh-client-plugin-{name}-{}", std::process::id()));
    ...
    #[test]
    fn discovers_only_declared_bounded_web_client_exports() {
        let profile = root("valid");
        std::fs::write(
            profile.join("package.json"),
            r#"{"dependencies":{"safe":"1","host-only":"1"}}"#,
        )
        .unwrap();
        for name in ["safe", "host-only"] {
            std::fs::create_dir_all(profile.join("node_modules").join(name)).unwrap();
        }
        std::fs::write(
            profile.join("node_modules/safe/client.js"),
            "window.safe=true",
        )
        .unwrap();
        std::fs::write(profile.join("node_modules/safe/package.json"), r#"{"exports":{"./client":"./client.js"},"dsh":{"client":{"platform":"web","inject":["base"]}}}"#).unwrap();
        std::fs::write(
            profile.join("node_modules/host-only/package.json"),
            r#"{"dsh":{"bundle":{"patch":"./cordis.patch.yml"}}}"#,
        )
        .unwrap();
        let plugins = discover(&profile).unwrap();
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].id, "safe");
        assert_eq!(plugins[0].inject, ["base"]);
        let _ = std::fs::remove_dir_all(profile);
    }

    #[test]
    fn rejects_client_exports_that_escape_the_package() {
        let profile = root("escape");
        std::fs::write(
            profile.join("package.json"),
            r#"{"dependencies":{"bad":"1"}}"#,
        )
        .unwrap();
        std::fs::create_dir_all(profile.join("node_modules/bad")).unwrap();
        std::fs::write(profile.join("node_modules/outside.js"), "bad").unwrap();
        std::fs::write(
            profile.join("node_modules/bad/package.json"),
            r#"{"exports":{"./client":"../outside.js"},"dsh":{"client":{"platform":"web"}}}"#,
        )
        .unwrap();
        assert!(discover(&profile).unwrap().is_empty());
        let _ = std::fs::remove_dir_all(profile);
    }
}
