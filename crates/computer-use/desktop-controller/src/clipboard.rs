...
fn open() -> Result<Open, String> {
    if unsafe { OpenClipboard(std::ptr::null_mut()) } == 0 {
        Err("COMPUTER_USE_CLIPBOARD_BUSY".into())
    } else {
        Ok(Open)
    }
}
pub fn read() -> Result<String, String> {
    ...
    unsafe {
        ...
        if memory.is_null() {
            return Err("COMPUTER_USE_CLIPBOARD_TEXT_UNAVAILABLE".into());
        }
        let size = GlobalSize(memory);
        if size > 131074 {
            return Err("COMPUTER_USE_CLIPBOARD_TOO_LARGE".into());
        }
        let pointer = GlobalLock(memory) as *const u16;
        if pointer.is_null() {
            return Err("COMPUTER_USE_CLIPBOARD_READ_FAILED".into());
        }
...
pub fn write(text: &str) -> Result<(), String> {
    if text.len() > 65536 || text.contains('\0') {
        return Err("COMPUTER_USE_CLIPBOARD_TEXT_INVALID".into());
    }
    let units = text
        .encode_utf16()
    ...
    unsafe {
        ...
        if memory.is_null() {
            return Err("COMPUTER_USE_CLIPBOARD_WRITE_FAILED".into());
        }
