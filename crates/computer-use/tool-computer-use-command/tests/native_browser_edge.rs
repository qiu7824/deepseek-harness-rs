...
async fn edge_controls_isolated_pages_and_returns_real_screenshots() {
    ...
    let modified = request(
        &adapter,
        json!({"action":"click","sessionId":"first","x":60,"y":45,"keys":["CTRL"]}),
    )
    .await;
    ...
    assert_eq!(
        trace,
        json!([[420, 220, true], [650, 240, true], [450, 430, true]])
    );
    let moved = request(
        &adapter,
        json!({"action":"move","sessionId":"first","x":500,"y":350}),
    )
    .await;
