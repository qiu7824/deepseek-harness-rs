//! Package-owned invariant companion for the bash seam: a no-op
//! registration — this stateless Service Definition owns request/result
//! types, while executors and policy own observations. Rust port of
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-shell";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "shell-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the shell invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct ShellInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for ShellInvariantPlugin {
