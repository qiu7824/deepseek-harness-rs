//! Package-owned invariant companion for `dsh-bash-local`: a no-op
...
//! port of `packages/shell/bash-local/src/invariant.ts`.
...
pub const PACKAGE_NAME: &str = "@deepseek-ai/dsh-bash-local";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "bash-local-invariant";
...
pub fn apply(ctx: &Context) -> cordis::Disposer {
    let registry = ctx
        ...
        .expect("the bash-local invariant companion requires the invariants service");
    registry.register(ctx, PACKAGE_NAME, installer())
}

/// The Cordis plugin form of the companion.
pub struct BashLocalInvariantPlugin;

#[async_trait::async_trait]
impl Plugin for BashLocalInvariantPlugin {
