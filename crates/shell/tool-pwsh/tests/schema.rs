...
async fn shipped_schema_mounts_and_timeout_bounds_are_enforced_before_execution() {
    ...
    let missing = std::env::temp_dir().join(format!("dsh-schema-missing-pwsh-{}.exe", std::process::id()));
    assert!(!missing.exists());
    dsh_pwsh_local::LocalPwshExecutor::install(&ctx, dsh_pwsh_local::Config {
        pwsh_path: Some(missing.to_string_lossy().into_owned()), ..Default::default()
    });
    dsh_tool_pwsh::ToolPwshService::install(&ctx).unwrap();
    for (timeout, expected) in [(0, "TOOL_INPUT_INVALID"), (600_001, "TOOL_INPUT_INVALID"), (3000, "SHELL_STARTUP_FAILED")] {
        ...
        assert_eq!(result.error.as_ref().unwrap().info.as_ref().unwrap().code, expected);
    }
}
