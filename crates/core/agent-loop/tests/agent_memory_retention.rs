...
async fn competing_factories_reserve_storage_before_setup_and_release_failed_resumes() {
    ...
    let blocked = b
        .resume(
            ...
            dsh_agent::ResumeAgentOptions {
                resume_session_id: Some(id.clone()),
                agent_options: Some(options()),
                setup: Some(counting_setup.clone()),
            },
        )
        .await
        .unwrap_err();
    ...
    assert!(b_sessions.list().is_empty());
    assert!(b_agents.list().is_empty());
    first.dispose.await;
    drop(first.agent);
    ...
    let failed = b
        .resume(
            ...
            dsh_agent::ResumeAgentOptions {
                resume_session_id: Some(id.clone()),
                agent_options: Some(options()),
                setup: Some(Arc::new(|_, _| {
                    Box::pin(async { Err("controlled setup failure".into()) })
                })),
            },
        )
        .await
    ...
    recovered.dispose.await;
    drop(recovered.agent);
