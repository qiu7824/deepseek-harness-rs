...
use dsh_subagent::continuation::{
    ContinuableStartSpec, SubagentFollowupOptions, SubagentInterruptAuthority,
};
...
fn spec(parent: Arc<dyn Agent>) -> ContinuableStartSpec {
    ContinuableStartSpec {
        provider: "spawn".into(),
        label: "quota child".into(),
        signal: Arc::new(|| false),
        request: SubagentStartRequest {
            label: Some("quota child".into()),
...
fn followup() -> SubagentFollowupOptions {
...
async fn real_creation_nested_admission_and_cold_resume_share_resident_slots() {
    let root =
        std::env::temp_dir().join(format!("resident-quota-runtime-{}", uuid::Uuid::new_v4()));
    let ctx = Context::root();
    dsh_system_prompt::SystemPrompt::install(&ctx, Default::default()).unwrap();
    let llm = LlmRuntime::install(&ctx);
    llm.register_adapter(&ctx, vec!["quota".into()], Arc::new(PendingAdapter))
        .unwrap();
    ...
    let parent = agent_loop
        .create_agent(
            &ctx,
            CreateAgentOptions {
                session_id: Some(session_id("quota-parent")),
    ...
    dsh_subagent_spawn_in_process::apply(&ctx, &Default::default()).unwrap();
    runtime.set_max_active_subagents(2).unwrap();
    let defaults = Arc::new(dsh_subagent::SubagentDefaults::from_strings(
        "quota", "model", "", 1024.0, 8.0, 3.0, 30.0,
    ));
    ctx.register_service(defaults.clone());
    let first = runtime
        .start_continuable(spec(parent.agent.clone()))
        .await
        .unwrap();
    let second = runtime
        .start_continuable(spec(parent.agent.clone()))
    ...
    runtime
        .suppress_settlement(&first.child_id, &parent.agent)
        .unwrap();
    runtime
        .interrupt(
            &first.child_id,
            &SubagentInterruptAuthority::User {
                parent_session_id: parent.agent.id().clone(),
            },
        )
        .unwrap();
    tokio::time::timeout(Duration::from_secs(5), async {
        while agents.get(&first.child_id).is_some() {
    ...
    assert_eq!(
        runtime
            .followup(parent.agent.clone(), &first.child_id, &content, followup())
            .await
            .unwrap_err()
            .code,
        "ACTIVATION_LIMIT_REACHED"
    );
    assert!(agents.get(&first.child_id).is_none());
    assert!(agents.get(&second.child_id).is_some());
    defaults.update(dsh_subagent::SubagentDefaults::from_strings(
        "changed",
        "different",
        "",
        2048.0,
    ...
    tokio::time::timeout(
        ...
        runtime.drain_continuable_descendants(&[parent.agent.clone()]),
    )
    .await
    .unwrap()
    .unwrap();
...
async fn calls_at_least(adapter: &CountingPending, n: usize) {
    tokio::time::timeout(Duration::from_secs(3), async {
        while adapter.0.load(Ordering::SeqCst) < n {
...
async fn parallel_limit_enforces_real_turns_and_idle_followup_reacquires() {
    ...
    dsh_system_prompt::SystemPrompt::install(&ctx, Default::default()).unwrap();
    let llm = LlmRuntime::install(&ctx);
    let adapter = Arc::new(CountingPending(AtomicUsize::new(0)));
                ...
                .create_agent(
                    ...
                    CreateAgentOptions {
                        ...
                        meta: Some(dsh_session::CreateSessionMeta {
                            ...
                            parent_session: Some(session_id("root")),
                            ..Default::default()
                        }),
                        agent_options: Some(dsh_agent::AgentOptions {
                            provider: Some("quota".into()),
    ...
    let prompt = || {
        dsh_llm::create_user_message(
            vec![ContentBlock::Text { text: "run".into() }],
            MessageSource::User {
                rpc_id: None,
                client_time_zone: None,
            },
    ...
    children[0].agent.followup(prompt());
    calls_at_least(&adapter, 1).await;
    children[1].agent.followup(prompt());
    tokio::time::timeout(Duration::from_secs(3), children[1].agent.when_idle())
        .await
        .unwrap();
    ...
    assert!(
        children[1]
            ...
            .events()
            .iter()
            .any(|e| e.type_ == "turn/end" && e.data.to_string().contains("maxParallel exceeded"))
    );
    runtime.set_max_parallel(2).unwrap();
    children[1].agent.followup(prompt());
    calls_at_least(&adapter, 2).await;
    runtime.set_max_parallel(1).unwrap();
    assert!(
        children
            .iter()
            .all(|c| c.agent.status() == dsh_agent::AgentStatus::Running)
    );
    ...
    children[1].agent.followup(prompt());
    calls_at_least(&adapter, 3).await;
    for child in children {
        child
            .agent
            .cancel(dsh_session::AgentCancelCause::User, None);
