...
async fn step_budget_stops_tool_cycles_and_resets_on_followup() {
    let h = harness_with_options(AgentOptions {
        max_steps: Some(2),
        ..Default::default()
    })
    .await;
    ...
    h.tools
        .register(&h.ctx, quick_tool(Arc::new(AtomicBool::new(false))))
        .unwrap();
    for expected in [2, 4] {
        h.agent
            .send(message("run"), dsh_agent::InboxTarget::NextTurn, true);
    ...
    assert!(
        events
            ...
            .all(|e| e.data.to_string().contains("maxTurns exceeded"))
    );
}
...
async fn deadline_cancels_stalled_model_and_allows_a_fresh_followup() {
    ...
    assert!(
        h.agent
            ...
            .iter()
            .filter(|e| e.type_ == "turn/end")
            .all(|e| e.data.to_string().contains("timeoutSeconds exceeded"))
    );
}
...
async fn deadline_cancels_uncooperative_tool_and_preserves_step_closure() {
    ...
    h.tools
        .register(&h.ctx, hanging_tool(entered.clone(), dropped.clone()))
        .unwrap();
...
async fn deadline_interrupts_a_stalled_pre_step_hook() {
    ...
    h.ctx
        .on("agent/pre-step", listener, Default::default())
        .await;
    h.agent.followup(message("run"));
    tokio::time::timeout(Duration::from_secs(3), h.agent.when_idle())
        .await
