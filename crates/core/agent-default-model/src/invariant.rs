...
//!
//! No runtime invariant: settings registration already validates every
//! mutable value before `currentSelection()` can observe it.
...
const PACKAGE_NAME: &str = "@deepseek-ai/dsh-agent-default-model";

/// Cordis companion plugin name (TS `name`).
pub const NAME: &str = "agent-default-model-invariant";
...
pub fn apply(ctx: &Context) -> BoxFuture<'static, Disposer> {
    ...
    Box::pin(async move {
        let invariants = ctx
            ...
            .expect("invariants service required by agent-default-model-invariant");
        invariants.register(
            &ctx,
            PACKAGE_NAME,
            InvariantInstaller {
