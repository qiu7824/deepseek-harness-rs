...
async fn failure_preserves_execution_receipt_without_becoming_success() {
    ...
    tools
        .register(
            ...
            ToolDefinition {
                name: "bounded-operation".into(),
                description: "receipt regression".into(),
                ...
                execute: Arc::new(|_, _| {
                    Box::pin(async {
                        Err(
                            ToolBodyError::coded("failed", "ExecutionError", "SHELL_FAILED")
                                .with_receipt(
                                    serde_json::json!({"exitCode":101,"processState":"exited"}),
                                ),
                        )
                    })
                }),
...
async fn late_blocking_worker_cannot_start_effects_after_no_effect_result() {
    ...
    .await;
    let (worker_ready, worker_finished, worker_release, worker_writes) =
        (ready.clone(), finished.clone(), receive, writes.clone());
    tools
        .register(
            &ctx,
            ToolDefinition {
                name: "bounded-operation".into(),
                description: "owned blocking effect fixture".into(),
                parameters: serde_json::json!({"type":"object"}),
                output: ToolOutputDefinition {
                ...
                execute: Arc::new(move |_, run| {
                    ...
                    let (ready, finished, release, writes) = (
                        worker_ready.clone(),
                        worker_finished.clone(),
                        worker_release.clone(),
                        worker_writes.clone(),
                    );
                    Box::pin(async move {
    ...
    assert_eq!(
        ...
        0,
        "late blocking work wrote after a no-effects result"
    );
