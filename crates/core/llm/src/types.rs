...
mod tests {
    ...
    fn content_block_wire_shape() {
        ...
        assert_eq!(
            json,
            serde_json::json!({"type": "tool-call", "id": "c1", "name": "run", "arguments": "{}"})
        );
    ...
    fn stream_chunk_wire_shape() {
        ...
        assert_eq!(
            serde_json::to_value(&chunk).unwrap(),
            serde_json::json!({"type": "tool-call-delta", "index": 3, "id": "c1", "name": "run", "argumentsDelta": "{\"a\":"})
        );
    }

    #[test]
    fn token_usage_and_failure_shapes() {
        ...
        let failure = LlmFailure {
            ...
            status: Some(429),
            provider_retry_after_ms: Some(1000),
            request_id: Some(crate::brand::provider_request_id("req-1")),
        };
        assert_eq!(
    ...
    #[test]
    fn call_config_round_trip() {
        let config = LlmCallConfig {
            provider: "deepseek".to_string(),
            model: "deepseek-chat".to_string(),
            reasoning_effort: Some(crate::brand::reasoning_effort_id("high")),
            temperature: Some(0.7),
            max_tokens: Some(4096),
            stop: Some(vec!["</answer>".to_string()]),
        };
        ...
        let back: LlmCallConfig = serde_json::from_value(json).unwrap();
