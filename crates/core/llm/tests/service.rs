...
async fn routes_stream_to_registered_adapter() {
    ...
    runtime
        .register_adapter(&ctx, vec!["test-provider".to_string()], Arc::new(ScriptedAdapter { script: script() }))
        .expect("register");
...
async fn adapter_dispatch_failure_becomes_failure_chunk() {
    ...
    runtime
        .register_adapter(
            &ctx,
            vec!["test".to_string()],
            Arc::new(ThrowingAdapter { message: "dispatch failed".to_string() }),
        )
        .expect("register");
...
async fn aborted_signal_maps_adapter_failure_to_aborted() {
    ...
    runtime
        .register_adapter(
            &ctx,
            vec!["test".to_string()],
            Arc::new(ThrowingAdapter { message: "stopped".to_string() }),
        )
        .expect("register");
...
async fn captures_provider_retry_policy_and_defaults_omission() {
    let ctx = Context::root();
    let runtime = LlmRuntime::install(&ctx);
    let configured = resolve_retry_policy(Some(&serde_json::json!({ "mode": "always" })), "test retryPolicy")
    ...
    impl LlmAdapter for PolicyAdapter {
        fn provider_retry_policy(&self, provider: &str) -> Option<ResolvedRetryPolicy> {
            (provider == "configured").then(|| resolve_retry_policy(Some(&serde_json::json!({ "mode": "always" })), "p").expect("configured"))
        }
        fn stream(&self, _options: &GenerateOptions) -> ChunkStream {
            Box::pin(futures::stream::iter(script()))
        }
    ...
    assert_eq!(runtime.provider_retry_policy("configured").expect("policy"), configured);
...
async fn prepared_call_pins_registration_across_route_replacement() {
    let ctx = Context::root();
    let runtime = LlmRuntime::install(&ctx);
    let old_policy = resolve_retry_policy(Some(&serde_json::json!({ "mode": "always" })), "old").expect("old policy");
    ...
    let prepared = runtime
        .prepare_call(&LlmCallConfig { provider: "route".to_string(), model: "model".to_string(), ..LlmCallConfig::default() }, None)
        .await
        .expect("prepare");
    ...
    runtime
        .register_adapter(
            &ctx,
            vec!["route".to_string()],
            Arc::new(PolicyAdapter { policy: new_policy.clone(), script: script(), throwing: false }),
        )
        .expect("register");
    ...
    assert_eq!(runtime.provider_retry_policy("route").expect("policy"), new_policy);
}
...
async fn prepared_call_rejects_config_drift_and_second_dispatch() {
    ...
    runtime
        .register_adapter(
            ...
            Arc::new(CatalogAdapter {
                provider: LlmProviderInfo { id: "route".to_string(), name: "Route".to_string() },
                models: Vec::new(),
    ...
    let prepared = runtime
        ...
        .await
        .expect("prepare");
    assert_eq!(prepared.adapter_defaults, LlmCallConfigAdapterDefaults { reasoning_effort: Some(true), max_tokens: None });
    ...
    let error = (prepared.stream)(options_for(&prepared.config)).err().expect("reuse must reject");
    assert_eq!(error.code, "INVALID_PREPARED_CALL");
}
...
async fn reuses_one_exact_model_lookup_for_prepared_config_and_context() {
        ...
        .register_adapter(
            ...
            Arc::new(CatalogAdapter {
                ...
                contexts: HashMap::from([(
                    "model".to_string(),
                    LlmModelContext { context_window: 128_000 },
                )]),
                reasoning: HashMap::from([(
                    "model".to_string(),
                    LlmModelReasoningInfo {
    ...
    let prepared = runtime
        ...
        .await
        .expect("prepare");
    assert_eq!(prepared.config.reasoning_effort, Some(reasoning_effort_id("high")));
    assert_eq!(prepared.context, Some(LlmModelContext { context_window: 128_000 }));
    collect((prepared.stream)(options_for(&prepared.config)).expect("dispatch")).await;
...
async fn materializes_reasoning_and_max_tokens_defaults() {
    ...
    let resolved = runtime
        .resolve_call_config(&LlmCallConfig { provider: "route".to_string(), model: "model".to_string(), ..LlmCallConfig::default() }, None)
        .await
        .expect("resolve");
    assert_eq!(
        resolved,
    ...
    assert_eq!(
        runtime.resolve_call_config(&explicit, None).await.expect("resolve"),
        explicit
    );
    let prepared_explicit = runtime.prepare_call(&explicit, None).await.expect("prepare");
    assert_eq!(prepared_explicit.adapter_defaults, LlmCallConfigAdapterDefaults::default());
}
...
async fn rejects_invalid_exact_model_metadata() {
    ...
    runtime
        .register_adapter(&ctx, vec!["mismatch".to_string()], Arc::new(MismatchAdapter))
        .expect("register");
    let error = runtime
    ...
    runtime
        .register_adapter(
            ...
            Arc::new(CatalogAdapter {
                provider: LlmProviderInfo { id: "invalid".to_string(), name: "Invalid".to_string() },
                models: Vec::new(),
                contexts: HashMap::from([("ctx".to_string(), LlmModelContext { context_window: 0 })]),
                reasoning: HashMap::from([
                    ("empty".to_string(), LlmModelReasoningInfo { efforts: Vec::new(), default_effort: None }),
                    (
                        "dup".to_string(),
                        LlmModelReasoningInfo {
                            efforts: vec![
    ...
    for (model, code) in [
        ...
        let error = runtime.resolve_model_info("invalid", model, None).await.expect_err(model);
...
async fn defaults_adapters_to_route_name_and_empty_catalog() {
    ...
    let runtime = LlmRuntime::install(&ctx);
    runtime
        .register_adapter(&ctx, vec!["plain".to_string()], Arc::new(ScriptedAdapter { script: script() }))
        .expect("register");

    assert_eq!(runtime.list_providers(), vec![LlmProviderInfo { id: "plain".to_string(), name: "plain".to_string() }]);
    assert!(runtime.list_models("plain").await.expect("models").is_empty());
    assert_eq!(error_code(&runtime.list_models("missing").await), "NO_ADAPTER");
    assert_eq!(
        runtime.resolve_model_info("plain", "unlisted", None).await.expect("resolve"),
        LlmResolvedModelInfo {
...
async fn rejects_invalid_or_duplicate_model_catalog_metadata() {
    ...
    runtime
        ...
        .expect("register");
    assert_eq!(error_code(&runtime.list_models("route").await), "INVALID_CATALOG");
    ...
    runtime
        .register_adapter(
            ...
            vec!["dup".to_string()],
            Arc::new(CatalogAdapter {
                provider: LlmProviderInfo { id: "dup".to_string(), name: "Dup".to_string() },
                models: vec![
                    LlmModelInfo {
        ...
        .expect("register");
    assert_eq!(error_code(&runtime.list_models("dup").await), "INVALID_CATALOG");
}
...
async fn llm_stream_listener_wraps_underlying_stream() {
    ...
    let listener: Arc<cordis::Listener> = Arc::new(|_ctx, args| {
        let next = cordis::downcast_arc::<cordis::NextFn>(&args[1]).expect("next").clone();
        Box::pin(async move {
            let value = next.call().await;
            let inner = cordis::downcast_arc::<StreamFactory>(&value).expect("factory").as_ref().clone();
            let wrapped: StreamFactory = Arc::new(move |options| {
                let prefix = futures::stream::iter(vec![
                    StreamChunk::BlockStart { index: 99, block_type: "text".to_string() },
    ...
    ctx.on("llm/stream", listener, cordis::EventOptions::default()).await;
    ...
    assert_eq!(chunks[0], StreamChunk::BlockStart { index: 99, block_type: "text".to_string() });
}
...
async fn llm_stream_listener_routes_provider_before_adapter_selection() {
    ...
    runtime
        .register_adapter(
            ...
            Arc::new(RecordingAdapter { script: script(), last: Arc::clone(&last) }),
        )
        .expect("register");
...
async fn replay_state_kept_for_same_adapter_stripped_for_other_adapter() {
    ...
    let adapter: Arc<dyn LlmAdapter> =
        Arc::new(RecordingAdapter { script: script(), last: Arc::clone(&last_same) });
    runtime
        .register_adapter(
            &ctx,
    ...
    let historical_message = create_assistant_message(
        vec![ContentBlock::Text { text: "old response".to_string() }],
        ModelMessageSource {
    ...
    runtime
        .register_adapter(
            ...
            Arc::new(RecordingAdapter { script: script(), last: Arc::clone(&last_target) }),
        )
        .expect("register");
    collect(runtime.stream(GenerateOptions {
...
async fn duplicate_empty_and_invalid_adapter_registrations_reject_atomically() {
    ...
    let runtime = LlmRuntime::install(&ctx);
    runtime
        .register_adapter(&ctx, vec!["m1".to_string()], Arc::new(ScriptedAdapter { script: script() }))
        .expect("register");
    ...
    assert_eq!(
        error_code(&runtime.register_adapter(&ctx, Vec::new(), Arc::new(ScriptedAdapter { script: script() }))),
        "INVALID_ADAPTER"
    );
    assert_eq!(
        error_code(&runtime.register_adapter(&ctx, vec![String::new()], Arc::new(ScriptedAdapter { script: script() }))),
        "INVALID_ADAPTER"
    );
    assert_eq!(
    ...
    assert_eq!(runtime.list_providers(), vec![LlmProviderInfo { id: "m1".to_string(), name: "m1".to_string() }]);
}
...
async fn replace_routes_atomically_and_guard_disposed_registration() {
    ...
    assert_eq!(runtime.list_providers(), vec![LlmProviderInfo { id: "m2".to_string(), name: "m2".to_string() }]);

    (handle.dispose)().await;
    assert!(runtime.list_providers().is_empty());
    let error = (handle.replace)(vec!["leaked".to_string()]).err().expect("disposed replace must refuse");
...
async fn unregisters_adapters_when_owning_fiber_disposes() {
    ...
    let fiber = ctx.plugin(
        Arc::new(RegisteringPlugin { adapter: Arc::new(ScriptedAdapter { script: script() }) }),
        arc(()),
    );
    fiber.settle().await.expect("fiber settles");
    assert_eq!(
        runtime.list_providers(),
        vec![LlmProviderInfo { id: "scoped-model".to_string(), name: "scoped-model".to_string() }]
    );
    fiber.dispose().await;
    assert!(runtime.list_providers().is_empty());
}
