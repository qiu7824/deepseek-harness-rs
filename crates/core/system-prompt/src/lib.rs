...
mod tests {
    ...
    async fn built_in_sections_and_persona() {
        ...
        let service = install(
            &ctx,
            Config {
                persona: "You are DeepSeek Harness.".into(),
                ..default_config()
            },
        );
        ...
        let assembly2 = service2
            .assemble(&ctx2, &AssembleContext::default())
            .await
            .unwrap();
    ...
    async fn resolves_text_providers_against_context_per_call() {
        ...
        context
            .fields
            .insert("who".into(), serde_json::json!("alice"));
    ...
    async fn fiber_disposal_removes_contributions() {
        ...
        assert_eq!(
            before.variables.get("scoped_var"),
    ...
    async fn change_listener_throw_rolls_back_registration() {
        ...
        assert_eq!(
            threw.load(MemOrder::SeqCst),
            2,
            "registration attempt and rollback must each emit change"
        );
        let assembly = service
            .assemble(&ctx, &AssembleContext::default())
            .await
    ...
    async fn waterfall_short_circuit_and_complete_section_restore() {
        ...
        ctx.on(
            "system-prompt/assemble",
            Arc::new(|_ctx, _args| {
                Box::pin(async move { Some(arc(SharedAssembly::new(PromptAssembly::default()))) })
            }),
            EventOptions::default(),
        )
        .await;
        ...
        ctx2.on(
            ...
            Arc::new(|_ctx, args| {
                Box::pin(async move {
                    ...
                    assert!(
                        complete_present,
                        "complete section missing before waterfall"
                    );
                    shared.0.lock().sections.push(AssembledSection {
                        name: "late".into(),
                        text: "late".into(),
    ...
    async fn multiple_complete_sections_reject() {
        ...
        assert_eq!(
            error,
            "multiple complete prompt sections are active: \"first\", \"second\""
        );
    }
    ...
    async fn variables_resolve_per_assembly_and_live_iterate() {
            ...
            Arc::new(move |_assembly_ctx| {
                if !added_for_provider.swap(true, MemOrder::SeqCst) {
                    ...
                    service.variable(
                        &ctx,
                        "late",
                        Arc::new(|_ctx| Some("second value".to_string())),
                    );
                }
        ...
        let assembly = service
            ...
            .unwrap();
        assert_eq!(
            assembly.variables.get("first"),
            Some(&Some("first value".to_string()))
        );
        assert_eq!(
            assembly.variables.get("late"),
            Some(&Some("second value".to_string()))
        );
    }
    ...
    async fn variable_name_validation() {
        ...
        let assembly = service
            ...
            .unwrap();
        assert_eq!(
            assembly.variables.get("model"),
            Some(&Some("m1".to_string()))
        );
    }
    ...
    async fn variables_interpolate_in_persona_and_waterfall() {
        ...
        let service = install(
            &ctx,
            Config {
                persona: "You run on {{model}} in {{cwd}}.".into(),
                ..default_config()
            },
        );
        service.variable(
            &ctx,
            "model",
            Arc::new(|_ctx| Some("deepseek-v4".to_string())),
        );
    ...
    fn interpolation_edge_cases() {
        ...
        for text in ["{{{model}}}", "x {{a{b}} y {{model}}"] {
            ...
            .unwrap_err();
            assert!(
                error.contains("malformed prompt variable reference at"),
                "{error}"
            );
        }
        ...
        let error = render_prompt(&PromptAssembly {
            sections: vec![AssembledSection {
                name: "s".into(),
                text: "on {{constructor}}".into(),
            }],
            ..Default::default()
        })
        .unwrap_err();
        assert!(
            error.contains("unknown prompt variable \"{{constructor}}\""),
            "{error}"
        );
    ...
    async fn tool_order_rejects_unknown_reserved_and_bad_configs() {
        ...
        let service2 = install(
            &ctx2,
            Config {
                tool_order: Some(vec!["ghost".into(), TOOL_ORDER_REST.into()]),
                ..default_config()
            },
        );
        let error = service2
        ...
        let error = service3
            .assemble(&ctx3, &AssembleContext::default())
            .await
            .unwrap_err();
        assert_eq!(
            error,
        ...
        assert!(
            error.contains("must contain the \"<unlisted-tools>\" rest entry"),
            "{error}"
        );
        let error =
        ...
        let error =
            parse_config(&serde_json::json!({"toolOrder": ["bash", "bash", "<unlisted-tools>"]}))
                .unwrap_err();
    ...
    fn config_parse_applies_defaults() {
        ...
        assert_eq!(
            config.tool_order,
            Some(vec!["a".to_string(), "<unlisted-tools>".to_string()])
        );
