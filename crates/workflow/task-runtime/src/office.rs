...
pub fn validate_office_for_automation(bytes: &[u8], format: &str) -> Result<()> {
    ...
    let mut archive = zip::ZipArchive::new(Cursor::new(bytes))
        .map_err(|e| format!("Invalid DOCX package: {e}"))?;
    ...
    for index in 0..archive.len() {
        ...
        if name.ends_with(".xml") || name.ends_with(".rels") {
            ...
            let document =
                roxmltree::Document::parse(&xml).map_err(|e| format!("Invalid Office XML: {e}"))?;
            ...
            {
                return Err(
                    "External Office relationships require separate review before automation"
                        .into(),
                );
            }
            if name == main {
                has_document = matches!(
