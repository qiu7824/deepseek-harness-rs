...
`;
    ...
    function ContextJumpRail() {
      const [snapshot, setSnapshot] = React.useState({ scroll: null, rows: [], current: -1, rect: null });
      ...
      return ReactDOM.createPortal(React.createElement("nav", {
        className: "ctxjump-host",
        style: hostStyle,
        "aria-label": "对话翻页"
      }, React.createElement("div", { className: "ctxjump-rail" },
        React.createElement("div", { className: "ctxjump-line", "aria-hidden": "true" }),
        React.createElement("button", {
          type: "button", className: "ctxjump-edge", "data-edge": "top", title: "回到顶部",
          onClick: () => snapshot.scroll.scrollTo({ top: 0, behavior: "smooth" })
        }, "↑"),
        marks.map((mark, index) => React.createElement("button", {
        ...
        }, React.createElement("span", { className: "ctxjump-tip" },
          mark.title,
          React.createElement("span", { className: "ctxjump-position" }, `${index + 1} / ${marks.length}`)
        ))),
        React.createElement("button", {
