window.__ModuleLoader__.load({id:"dsh-voice-input",factory:require=>{
 ...
 function VoiceInputButton(props){
  ...
  React.useEffect(()=>{setListening(false);setError("");const documentRef=globalThis.document??globalThis.window?.document;const escape=globalThis.CSS?.escape||((value)=>String(value).replace(/[^a-zA-Z0-9_-]/g,"_"));const selector=`textarea[data-composer-input][data-session-id="${escape(props.sessionId)}"]`;const field=documentRef?.querySelector(selector);const clear=()=>{if(holdTimer.current!==null){clearTimeout(holdTimer.current);holdTimer.current=null}if(active.current)stop()};const down=event=>{if(event.button!==0||disabled)return;holdTimer.current=setTimeout(()=>{holdTimer.current=null;begin()},280)};field?.addEventListener("pointerdown",down);field?.addEventListener("pointerup",clear);field?.addEventListener("pointercancel",clear);field?.addEventListener("pointerleave",clear);return()=>{field?.removeEventListener("pointerdown",down);field?.removeEventListener("pointerup",clear);field?.removeEventListener("pointercancel",clear);field?.removeEventListener("pointerleave",clear);clear();generation.current++;held.current=false;const state=active.current;active.current=null;if(state){state.engine.onresult=state.engine.onerror=state.engine.onend=null;state.engine.abort()}}},[props.sessionId,disabled]);
