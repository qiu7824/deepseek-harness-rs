...
function TaskManager({ useProjection, useSession, updateTodos, sendInstruction, cancelTurn }) {
  ...
  const apply = async (action, index, item, value = '') => {
    ...
    setError('')
    try {
      const direct = subagent === null && await updateTodos(
        todos,
        action === 'edit'
          ? { kind: 'edit', index, content: value.trim() }
          : { kind: 'remove', index },
      )
      if (!direct) {
  ...
  const renderEditor = (item, index) => createElement('div', { className: 'dshtm-edit' },
    createElement('input', {
      autoFocus: true,
      className: 'dshtm-input',
      value: editing.value,
      'aria-label': '修改任务内容',
      onChange: (event) => setEditing({ ...editing, value: event.currentTarget.value }),
      onKeyDown: (event) => {
        if (event.key === 'Enter' && !event.nativeEvent.isComposing && editing.value.trim()) {
          apply('edit', index, item, editing.value)
        }
    ...
    createElement('button', {
      type: 'button',
      className: 'dshtm-action',
      title: '保存',
      disabled: busy !== null || !editing.value.trim(),
      onClick: () => apply('edit', index, item, editing.value),
    }, '✓'),
    createElement('button', {
      type: 'button',
      className: 'dshtm-action',
  ...
  const renderActions = (item, index) => createElement('div', { className: 'dshtm-actions' },
    createElement('button', {
      type: 'button',
      className: 'dshtm-action',
      title: '修改任务',
      disabled: busy !== null || !mutable,
      onClick: () => setEditing({ index, original: item.content, value: item.content }),
    }, '✎'),
    item.status !== 'completed' && createElement('button', {
      type: 'button',
      className: 'dshtm-action',
      'data-danger': 'true',
      title: '停止并移除这个任务',
      disabled: busy !== null || !mutable,
      onClick: () => apply('stop', index, item),
    }, '■'),
  )

  const rows = todos.map((item, index) => createElement('li', {
    className: 'dshtm-row',
    ...
    editing?.index !== index && renderActions(item, index),
  ))

  const panel = open && createElement('section', {
    className: 'dshtm-popover',
    id: panelId,
    'aria-label': '任务清单管理',
  },
    ...
    todos.length === 0
      ? createElement('div', { className: 'dshtm-empty' }, '暂无任务；智能体创建任务清单后会自动显示进度。')
      : createElement('ul', { className: 'dshtm-list' }, rows),
    todos.length > 0 && createElement('div', { className: 'dshtm-foot' },
      ...
      running && createElement('button', {
        type: 'button',
        className: 'dshtm-stop-turn',
        disabled: busy !== null || !mutable,
        onClick: stopTurn,
      }, busy === 'turn' ? '正在停止…' : '停止当前运行'),
    ),
  )
  ...
  return createElement('div', {
    ...
    createElement('button', {
      type: 'button',
      className: 'dshtm-trigger',
      title: `任务清单：${summary}`,
      'aria-label': `任务清单，${summary}`,
      'aria-expanded': open,
      ...
      icon(),
      todos.length > 0 && createElement('span', {
        className: 'dshtm-badge',
        'aria-hidden': 'true',
      }, todos.length),
    ),
    panel,
      ...
      return {
        updateTodos: async (expected, action) => {
          ...
          const message = {
            ...
            payload: { sessionId, expected, action },
          }
