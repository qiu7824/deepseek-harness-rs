	...
	factory: (require) => {
			...
			*/
			select(selection, options = {}) {
                ...
                this.selectionPromise = (async () => {
                    await Promise.resolve();
                    try {
                        const { result } = await this.sessions.selectModel({sessionId:this.sessionId,provider:selection.provider,model:selection.model,executionMode:selection.executionMode??"standard",...selection.reasoningEffort===void 0?{}:{reasoningEffort:selection.reasoningEffort}});
                        if (!result.ok) throw new Error(`${result.error.code}: ${result.error.message}`);
						...
						try { if(options.saveDefault!==false) await this.saveDefault(result.value.selected); }
                        catch (error) {
				...
				const directory = new ModelDirectory(api.sessions, sessionId, () => sessions.subagentAddress(sessionId) === void 0, async (selection) => {
					const response = await api.settings.replace({
						...
						section: {
                            ...
                            executionMode: selection.executionMode ?? "standard",
		...
		*/
		...
		function ModelSelect({ locked, available, directory, load, select, t }) {
			...
			const filteredGroups=(0,react.useMemo)(()=>{const needle=modelQuery.trim().toLocaleLowerCase();if(!needle)return state.groups;return state.groups.map(group=>({...group,models:group.models.filter(model=>(group.name+" "+model.name+" "+model.id).toLocaleLowerCase().includes(needle))})).filter(group=>group.models.length>0)},[state.groups,modelQuery]);
		...
		function selectionOf(state, id) {
			for (const group of state.groups) for (const model of group.models) {
				...
				return {
					provider: group.id,
					model: model.id,
					executionMode: state.current?.provider === group.id && state.current?.model === model.id ? state.current.executionMode ?? "standard" : "standard",
