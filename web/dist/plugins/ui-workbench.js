window.__ModuleLoader__.load({
 ...
 factory:(require)=>{
  ...
  function nodeStatusText(node){return ({ready:"可用",missing:"未安装",incompatible:"版本或能力不兼容",error:"检测失败",timeout:"检测超时"})[node?.status]||"尚未检测"}
  function ContextSettings({api,renderSlot}) {
   ...
   const load=React.useCallback(async()=>{
    try {
     const [h,s,w,response]=await Promise.all([api.host.describe({}),api.settings.describe({}),api.workspace.list({}),fetch("/__dsh-runtime?refreshNode=1")]);
   ...
   const refreshNode=()=>act(async()=>{const response=await fetch("/__dsh-runtime?refreshNode=1",{cache:"no-store"});const runtime=await response.json();if(!response.ok)throw new Error(runtime.error||"检测失败");setState(x=>({...x,runtime}));});
   ...
   return jsx.jsxs("section",{className:"dsh-env",children:[
    ...
    jsx.jsxs("div",{className:"dsh-env-section",children:[jsx.jsx("div",{className:"dsh-env-head",children:"Rust 核心"}),jsx.jsx("div",{className:"dsh-env-grid",children:[["Host 版本",state.host?.version],["活动会话",String(state.host?.attachedSessions??0)],["Provider",state.host?.provider??"未设置"],["模型",state.host?.model??"未设置"],["工作区",String(state.workspaces.length)]].map(([k,v])=>jsx.jsxs("div",{className:"dsh-env-stat",children:[jsx.jsx("div",{className:"dsh-env-k",children:k}),jsx.jsx("div",{className:"dsh-env-v",title:v,children:v})]},k))}),jsx.jsx("div",{className:"dsh-env-note",children:"普通会话、原生工具与预构建的 Web 界面由 Rust Host 运行，无需 Node.js。"})]}),
    jsx.jsxs("div",{className:"dsh-env-section",children:[jsx.jsx("div",{className:"dsh-env-head",children:"PTC 代码模式运行依赖 · Node.js"}),jsx.jsx("div",{className:"dsh-env-grid",children:[["状态",nodeStatusText(state.runtime?.node)],["版本",state.runtime?.node?.version||"未检测"],["来源",state.runtime?.node?.source==="environment"?"运行环境目录":state.runtime?.node?.source==="path"?"系统 PATH":"未检测"]].map(([k,v])=>jsx.jsxs("div",{className:"dsh-env-stat",children:[jsx.jsx("div",{className:"dsh-env-k",children:k}),jsx.jsx("div",{className:"dsh-env-v",title:v,children:v})]},k))}),pathRow("实际运行程序",state.runtime?.node?.path),jsx.jsxs("div",{className:"dsh-env-note",children:[state.runtime?.node?.capabilities&&jsx.jsx("p",{children:[["permission","权限模型"],["typescriptStrip","TypeScript"],["worker","工作线程"]].map(([key,label])=>label+"："+(state.runtime.node.capabilities[key]===true?"已验证":"未通过")).join(" · ")}),state.runtime?.node?.error&&jsx.jsx("p",{role:"status",children:state.runtime.node.error}),jsx.jsx("p",{children:"PTC 代码模式执行 JavaScript / TypeScript 工具编排，必须使用兼容的 Node.js；这是该功能的运行依赖。标准模式可直接使用 Rust 原生工具。部分 MCP 和项目工具还有各自的运行依赖。"}),jsx.jsx("button",{disabled:busy,onClick:refreshNode,children:busy?"检测中…":"重新检测"})]})]}),
    jsx.jsxs("div",{className:"dsh-env-section",children:[jsx.jsx("div",{className:"dsh-env-head",children:"存储目录"}),pathRow("Host 启动目录",state.host?.cwd),...[["正式数据根","dataDirectory"],["缓存目录","cacheDirectory"],["运行环境目录","environmentDirectory"],["测试运行目录","testDirectory"]].map(([label,field])=>pathRow(label,state.runtime?.paths?.[field],field)),jsx.jsxs("div",{className:"dsh-env-note",children:[jsx.jsx("button",{"data-primary":true,disabled:busy||!changed||Object.values(draft).some(value=>!String(value).trim()),onClick:save,children:busy?"处理中…":"保存目录设置"})," ",jsx.jsx("button",{disabled:busy||changed||!state.runtime?.restartSupported,onClick:restart,children:"重启并应用"}),jsx.jsx("p",{children:"运行环境目录中的 node 或 bin/node（Windows 为 node.exe）优先用于代码模式；否则查找系统 PATH。目录更改重启后重新检测。工作区文件保持在各自项目目录。"})]})]}),
  ...
  return {apply,inject:["slots","connection"],nodeStatusText}
 }
});
