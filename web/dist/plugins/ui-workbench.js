 ...
 factory:(require)=>{
  ...
  function ContextSettings({api,renderSlot}) {
   ...
   return jsx.jsxs("section",{className:"dsh-env",children:[
    ...
    jsx.jsxs("div",{className:"dsh-env-section",children:[jsx.jsx("div",{className:"dsh-env-head",children:"可选代码执行环境 · Node.js"}),jsx.jsx("div",{className:"dsh-env-grid",children:[["状态",nodeStatusText(state.runtime?.node)],["版本",state.runtime?.node?.version||"未检测"],["来源",state.runtime?.node?.source==="environment"?"运行环境目录":state.runtime?.node?.source==="path"?"系统 PATH":"未检测"]].map(([k,v])=>jsx.jsxs("div",{className:"dsh-env-stat",children:[jsx.jsx("div",{className:"dsh-env-k",children:k}),jsx.jsx("div",{className:"dsh-env-v",title:v,children:v})]},k))}),pathRow("实际运行程序",state.runtime?.node?.path),jsx.jsxs("div",{className:"dsh-env-note",children:[state.runtime?.node?.capabilities&&jsx.jsx("p",{children:[["permission","权限模型"],["typescriptStrip","TypeScript"],["worker","工作线程"]].map(([key,label])=>label+"："+(state.runtime.node.capabilities[key]===true?"已验证":"未通过")).join(" · ")}),state.runtime?.node?.error&&jsx.jsx("p",{role:"status",children:state.runtime.node.error}),jsx.jsx("p",{children:"代码模式使用 Node；部分 MCP 和项目工具也有各自的运行依赖。检测仅检查当前代码环境，不安装软件。"}),jsx.jsx("button",{disabled:busy,onClick:refreshNode,children:busy?"检测中…":"重新检测"})]})]}),
