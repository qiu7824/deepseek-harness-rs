window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const css = ".xkMHRq_row{border-bottom:1px solid var(--dsw-alias-border-l2);align-items:center;gap:8px;padding:16px 0;display:flex}.xkMHRq_rowText{flex-direction:column;flex:1;gap:4px;min-width:0;padding-right:48px;display:flex}.xkMHRq_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}.xkMHRq_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;font-weight:400;line-height:18px}.xkMHRq_selector{background:var(--dsw-alias-bg-module-platform);height:36px;font:inherit;color:var(--dsw-alias-label-primary);cursor:pointer;border:none;border-radius:18px;align-items:center;gap:12px;padding:0 14px;font-size:14px;line-height:22px;display:inline-flex}.xkMHRq_selector:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}.xkMHRq_selector:disabled{cursor:default}.xkMHRq_chevron{flex:none}";
		...
		var PermissionRow_module_css_default = {
			"title": "xkMHRq_title",
			"selector": "xkMHRq_selector",
			"chevron": "xkMHRq_chevron",
			"desc": "xkMHRq_desc",
			"row": "xkMHRq_row",
			"rowText": "xkMHRq_rowText"
		};
		//#endregion
		...
		*/
		...
		const accessZh = {
			"description.workspaceWrite": "可在工作区及允许的临时目录内写入；扩大范围的重试需要审批。",
			"description.fullAccess": "可访问全部文件，不弹出审批提示。",
			"preset.readOnly": "仅可查看",
			"preset.workspaceWrite": "工作区内修改",
			"preset.fullAccess": "完全权限",
			"confirm.title": "确认启用完全权限？",
		...
		const accessEn = {
			"description.workspaceWrite": "Write inside the workspace and permitted temporary directories; wider retries require approval.",
			"description.fullAccess": "Full file access without approval prompts.",
			"preset.readOnly": "Read Only",
			"preset.workspaceWrite": "Workspace Write",
			"preset.fullAccess": "Full access",
			"confirm.title": "Enable Full access?",
		...
		*/
        ...
        function permissionDescription(option, t) {
            ...
            if (option.value === "workspace-write" && description === "Write inside the workspace and permitted temporary directories; wider retries require approval.") return t("description.workspaceWrite");
            if (option.value === "danger-full-access" && description === "Full file access without approval prompts.") return t("description.fullAccess");
            return description;
        }
		function optionsOf(value, t) {
			return value.options.filter((option) => option.value !== "custom").map((option) => ({
				...
				...option.description !== void 0 ? { detail: permissionDescription(option, t) } : {},
