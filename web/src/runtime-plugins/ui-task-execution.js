window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    function TaskExecutionView({sessionId}) {
      ...
      React.useEffect(()=>{live.current=true;setTasks([]);setDetail(null);setSelected("");setPending(null);setConfirmation(null);setError("");load().catch(e=>{if(live.current&&e.name!=="AbortError")setError(e.message);});return()=>{live.current=false;generation.current++;controller.current?.abort();};},[load]);
      const perform=async payload=>{
        if(busy)return;setBusy(true);setError("");setPending(payload);
        try{await request(sessionId,payload);if(live.current){setPending(null);setConfirmation(null);await load(payload.taskId);}}
        catch(e){if(live.current)setError(e.message||String(e));}
      ...
      return h("section",{className:"dshTaskExecution","aria-label":"任务验收与恢复"},
        ...
        task&&h(React.Fragment,null,
          h("article",{"data-state":task.state},h("div",{className:"actions"},h("h3",null,task.spec.objective),h("span",{className:"badge"},labels[task.state]||task.state)),h("small",null,`任务 ${task.taskId} · 版本 ${task.revision}`),
            ...
            h("div",{className:"actions"},!terminal&&h("button",{disabled:busy,onClick:()=>act("validate")},"重新验收"),task.state==="validating"&&h("button",{disabled:busy||detail.blockers.length>0,onClick:()=>act("complete")},"核对版本并完成"),task.state==="cancelled"&&h("button",{disabled:busy,onClick:()=>act("resume")},"继续此任务"),!terminal&&h("button",{disabled:busy,onClick:()=>setConfirmation({action:"cancel"})},"取消任务")),
