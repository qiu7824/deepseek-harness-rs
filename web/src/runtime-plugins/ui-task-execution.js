window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    function revisionError(error){return ({TASK_BUSY:"任务仍在执行或验收，请先停止工作，再保存要求。",TASK_REVISION_CONFLICT:"任务版本已变化；草稿已保留，请核对最新要求。",TASK_IDEMPOTENCY_CONFLICT:"该保存标识已有不同内容，请先核对历史记录。",TASK_ENVIRONMENT_CHANGED:"执行环境已变化，请先完成环境切换，再保存要求。",TASK_SUCCESSOR_REQUIRED:"此任务已完成，请创建后续任务以保留历史。",TASK_ACTIVE_CONTRACT:"当前会话还有未结束的任务，暂时无法创建后续任务。",TASK_GOAL_REQUIREMENTS_CHANGED:"当前目标已变化；请核对并明确采用当前目标后再保存。",TASK_GOAL_DETACH_REQUIRES_SUCCESSOR:"已关联目标不能原地解除，请创建独立后续任务。"})[error.code]||error.message||String(error);}
    function TaskRequirementsEditor({sessionId,task,currentGoalRequirements,onSaved,onClose,onReload}){
      const writer=React.useRef(null),initial=React.useRef(null);if(!writer.current)writer.current=crypto.randomUUID();
      if(!initial.current){const records=draftRecords(sessionId,task.taskId),existing=records.find(row=>row.value.pendingSave)??records.find(row=>row.value.baseRevision===task.revision);initial.current={record:existing,records,value:freshDraft(existing?.value??{sessionId,taskId:task.taskId,baseRevision:task.revision,mode:task.state==="completed"?"successor":"in_place",expectedGoalBinding:task.goalBinding??null,contract:editorContract(task.spec)},writer.current)};}
      ...
      const save=async()=>{
        ...
        try{if(!payload)payload={action:"revise",taskId:task.taskId,expectedRevision:draft.baseRevision,idempotencyKey:crypto.randomUUID(),mode:draft.mode,expectedGoalBinding:draft.expectedGoalBinding??null,contract:savedContract(draft.contract)};}catch(error){setError(error.message);return;}
