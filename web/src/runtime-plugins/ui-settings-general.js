        ...
        function ExperiencePanel({api,sessionId,memoryEnabled}) {
            ...
            const refresh=react.useCallback(async({quiet=false,force=false}={})=>{
                ...
                try{
                    ...
                    const next=experienceList(experienceUnwrap(await api.memory.learningList({...workspaceKey?{workspaceKey}:{},...status?{status}:{},...cause?{cause}:{},...query.trim()?{query:query.trim()}:{},limit:compact&&!expanded?6:1000})));
            ...
            },[api,sessionId,compact,workspace,status,cause,query,expanded,memoryEnabled]);
            ...
            return h("section",{className:"dshMemory dshExperiencePanel","data-compact":compact||undefined,"data-experience-panel":compact?"context":"settings"},
                ...
                h("div",{className:"dshMemoryList"},rows.map(entry=>{
                    ...
                    return h("article",{key:entry.id,className:"dshMemoryItem","data-experience-id":entry.id},
                        ...
                        entry.review&&h("section",{"data-diagnostic-review":entry.review.cause},h("strong",null,({agent_implementation:"Agent 实现缺陷",agent_usage:"Agent 调用或业务脚本错误",environment:"运行环境或能力",provider:"提供方或账号条件",task_state:"任务状态",expected_control:"预期控制结果",mixed:"混合原因",insufficient_evidence:"证据不足"})[entry.review.cause]||entry.review.cause),h("span",null," · ",entry.reviewStale?"新增事件，需重新核对":({open:"未修复",fixed:"修复已验证",external:"外部条件",historical:"历史事件",unknown:"尚不能确定",mixed:"部分修复"})[entry.review.status]),h("p",null,entry.review.summary),h("details",null,h("summary",null,"核对证据"),h("ul",null,(entry.review.evidence||[]).map((item,index)=>h("li",{key:index},item))))),
