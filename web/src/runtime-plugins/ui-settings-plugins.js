	...
	factory: (require) => {
		...
		*/
        ...
        function PluginInstallControls(){
            ...
            const [spec,setSpec]=react.useState(""),[action,setAction]=react.useState("add"),[busy,setBusy]=react.useState(false),[confirm,setConfirm]=react.useState(false),[error,setError]=react.useState(""),[log,setLog]=react.useState("");
            const alive=react.useRef(false);react.useEffect(()=>{alive.current=true;return()=>{alive.current=false}},[]);
            const submit=async()=>{if(busy)return;setBusy(true);setError("");try{const response=await fetch("/__dsh-plugin-manager",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,spec:spec.trim()})});const value=await response.json();if(!response.ok)throw new Error(value.error||"插件操作失败");if(alive.current){setLog(value.log+"\n文件已更新，重启应用后生效。");setConfirm(false);}}catch(e){if(alive.current)setError(e.message)}finally{if(alive.current)setBusy(false)}};
            const h=react.createElement;
            return h("section",{className:"dshSettingsDisclosure dshPluginInstall",style:{marginBottom:20,maxWidth:760}},h("h3",null,"安装与移除"),h("p",null,"支持 Rust 的纯 Web 插件；带有 Node Host 功能的部分不会在 Rust 进程中执行。"),h("label",null,"操作",h("select",{value:action,disabled:busy,onChange:e=>{setAction(e.target.value);setConfirm(false)}},h("option",{value:"add"},"安装 / 更新"),h("option",{value:"remove"},"移除"))),h("label",null,action==="add"?"GitHub 来源":"已安装包名",h("input",{value:spec,disabled:busy,maxLength:400,placeholder:action==="add"?"github:owner/repo#commit":"@scope/package",onChange:e=>{setSpec(e.target.value);setConfirm(false)},style:{width:"100%",boxSizing:"border-box"}})),h("button",{type:"button",disabled:busy||!spec.trim(),onClick:()=>setConfirm(true)},busy?"正在处理…":"检查操作"),confirm&&h("div",null,h("p",null,action==="add"?"插件将在浏览器中运行；确认信任此来源并安装？":"移除后重启应用生效；确认移除此插件？"),h("button",{disabled:busy,onClick:submit},"确认"),h("button",{disabled:busy,onClick:()=>setConfirm(false)},"取消")),error&&h("p",{role:"alert"},error),log&&h("pre",{role:"status",style:{whiteSpace:"pre-wrap",overflowWrap:"anywhere"}},log));
        }
