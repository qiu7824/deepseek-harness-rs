  ...
  factory: require => {
    ...
    function SkillRevisionSection({rpc}) {
      ...
      return jsxs("section",{className:"dshSkillVersions",children:[
        ...
        state&&jsxs("label",{className:"dshSkillToggle",children:[jsx("input",{type:"checkbox",role:"switch",checked:state.enabled!==false,disabled:busy,onChange:e=>{const enabled=e.target.checked;act(async()=>{await mutate("Toggle",{enabled});setNotice(enabled?"技能候选已启用。":"已停用技能候选加载，历史与手动技能保持可用。");});}}),"启用技能候选与已验证版本"]}),
        jsxs("div",{className:"actions",children:[jsx("button",{disabled:busy||!state||state.enabled===false,onClick:()=>setDraft(empty()),children:"创建候选"}),jsx("button",{disabled:busy,onClick:()=>act(load),children:"刷新"})]}),
        !state&&!error&&jsx("p",{role:"status",children:"正在读取技能候选…"}),error&&jsx("p",{role:"alert",children:error}),notice&&jsx("p",{role:"status",children:notice}),
