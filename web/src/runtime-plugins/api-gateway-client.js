		...
		*/
		...
		var ClientRemoteEvents = class {
			...
			*/
			...
			deliver(frame) {
				return privateEvents(this.ownerCtx).parallel(this.eventKey(frame.event), ...frame.args).catch((error) => {
			...
			async pumpEvents(signal, ready) {
				...
				try {
					for await (const value of source) {
						...
						if (frame.type === "emit") {
							const delivery = this.deliver(frame).finally(() => tasks.delete(delivery));
							tasks.add(delivery);
							continue;
						}
