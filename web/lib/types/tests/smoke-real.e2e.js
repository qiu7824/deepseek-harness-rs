...
import { REPO_ROOT, connectFreshWorkspace, newEnglishPage, probeFreePort, requireDist, saveFailureShot } from "./support.js";
...
async function rpc(baseUrl, method, payload) {
    ...
    if (!response.ok)
        throw new Error(`${method} failed over HTTP ${response.status}: ${await response.text()}`);
...
function hasAssistantMarker(page, marker) {
    return page.events.some(({ event }) => {
        ...
        if (!Array.isArray(content))
            return false;
        return content.some(block => isRecord(block) && block.type === 'text' && typeof block.text === 'string' && block.text.includes(marker));
    });
}
async function history(baseUrl, sessionId) {
    return rpc(baseUrl, 'session.history', { sessionId, maxMessages: 10 });
}
async function waitForProviderTitle(baseUrl, sessionId) {
    let observed;
    ...
    if (observed === undefined)
        throw new Error('provider-backed session title was not observed');
    return observed;
}
...
async function firstTrack(page) {
    return (await page.locator('[class*="frame"]').evaluate(el => getComputedStyle(el).gridTemplateColumns)).split(' ')[0];
}
/** Last column track (details) as a number of pixels. */
async function detailsTrack(page) {
    const cols = await page.locator('[class*="frame"]').evaluate(el => getComputedStyle(el).gridTemplateColumns);
...
if (notReady.length > 0)
    console.warn(`[smoke-real] skipped — client bundles not ready: ${notReady.join(', ')}`);
describe('dsh web keyless CLI smoke', () => {
    ...
    it('routes web runtime context and workspace instructions through the real CLI request', async () => {
        ...
        let resolveProviderRequests;
        const requests = [];
        const providerRequests = new Promise((resolve) => {
        ...
        const provider = createServer((request, response) => {
            ...
            request.on('end', () => {
                ...
                if ((parsed.tools?.length ?? 0) > 0)
                    requests.push(parsed);
                if (requests.length === 1)
                    resolveProviderRequests(requests);
        ...
        try {
            ...
            const workspaceMessage = captured.messages?.find(message => message.role === 'user' && message.content?.includes('web-workspace-context-probe'));
    ...
    it('retries a partial transport failure through the shipped Web composition', async () => {
        ...
        try {
            ...
            if (page === undefined)
                throw new Error('retry history was not observed');
    ...
    it('DSH_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
        ...
        const provider = createServer((request, response) => {
            ...
            request.on('data', (chunk) => { body += chunk; });
            request.on('end', () => {
                resolveProviderRequest(JSON.parse(body));
                response.writeHead(200, { 'content-type': 'text/event-stream' });
                response.end([
                    'data: {"choices":[{"delta":{"role":"assistant","content":null,"reasoning_content":""}}]}',
                    'data: {"choices":[{"delta":{"content":"done"}}]}',
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
    ...
    afterAll(async () => {
        ...
        if (child !== undefined && child.exitCode === null) {
            const gone = new Promise(resolveExit => child.once('exit', () => { resolveExit(); }));
            child.kill('SIGTERM');
...
//# sourceMappingURL=smoke-real.e2e.js.map