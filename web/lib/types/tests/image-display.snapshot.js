...
async function openFixtureSession() {
    ...
    if (group === null || group === undefined)
        throw new Error('fixture Workspace group missing');
...
it('renders the history image pair through the authorized attachment route and opens the lightbox', async () => {
    ...
    const galleryShape = (align) => [...document.querySelectorAll(`[data-align="${align}"] img`)]
  ...
  `);
    ...
    if (frame === null)
        throw new Error('image frame button missing');
...
it('accepts pasted images into the composer rail in order and removes them', async () => {
    ...
    if (start === null)
        throw new Error('fixture Workspace new-session action missing');
  ...
  `);
    ...
    if (remove.length !== 2)
        throw new Error('remove buttons missing');
...
it('accepts a whole-page drop under the limits-labeled overlay and refuses an over-limit batch at intake', async () => {
    ...
    await waitFor(() => {
        ...
        if (rail === null)
            throw new Error('attachment rail missing after page drop');
    ...
    const batch = Array.from({ length: 20 }, (_, i) => new File([new Uint8Array([137, 80, 78, 71])], `bulk-${String(i)}.png`, { type: 'image/png' }));
    fireEvent.paste(textarea, {
        clipboardData: {
...
//# sourceMappingURL=image-display.snapshot.js.map