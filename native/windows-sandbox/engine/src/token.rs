...
/// Read the principal from the OS token; environment display names are not identity.
pub fn current_user_sid_string() -> Result<String> {
    let mut token = 0;
    if unsafe { windows_sys::Win32::System::Threading::OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &mut token) } == 0 {
        return Err(anyhow!("OpenProcessToken: {}", unsafe { GetLastError() }));
    }
    let sid = unsafe { get_user_sid_bytes(token) };
    unsafe { CloseHandle(token); }
    crate::winutil::string_from_sid_bytes(&sid?).map_err(anyhow::Error::msg)
}

pub fn setup_caller_is_restricted() -> Result<bool> {
    let mut token=0;
    if unsafe { windows_sys::Win32::System::Threading::OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &mut token) }==0 {
        return Err(anyhow!("cannot inspect setup caller token"));
    }
    let restricted=unsafe { windows_sys::Win32::Security::IsTokenRestricted(token) }!=0;
    unsafe{CloseHandle(token);}
    Ok(restricted)
}
