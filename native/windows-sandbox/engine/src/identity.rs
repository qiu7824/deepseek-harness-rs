...
fn load_users(codex_home: &Path) -> Result<Option<SandboxUsersFile>> {
    ...
    match serde_json::from_str::<SandboxUsersFile>(&file) {
        Ok(users) => {
            let (offline, online) = crate::setup::account_names(codex_home);
            anyhow::ensure!(users.offline.username == offline && users.online.username == online,
                "STATE_OWNERSHIP_MISMATCH: refusing another application's credentials");
            Ok(Some(users))
        },
        Err(err) => {
            debug_log(
...
pub fn require_logon_sandbox_creds(
    ...
    if identity.is_none() {
        ...
        anyhow::bail!("SETUP_REQUIRED: native sandbox identity needs explicit setup or repair; command not dispatched");
    }
...
pub(crate) fn refresh_logon_sandbox_creds(
    ...
    let _ = (permissions, command_cwd, env_map, codex_home, read_roots_override,
        read_roots_include_platform_defaults, write_roots_override, deny_read_paths_override,
        deny_write_paths_override, proxy_enforced, proxy_settings_mode);
    anyhow::bail!("SETUP_REQUIRED: native account login failed; explicit repair is required")
}
