...
pub fn account_names(home: &Path) -> (String, String) {
    use sha2::{Digest, Sha256};
    let home = canonicalize_path(home).to_string_lossy().to_lowercase();
    let hash = format!("{:x}", Sha256::digest(home.as_bytes()));
    (format!("DshOff_{}", &hash[..12]), format!("DshOn_{}", &hash[..12]))
}
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "dsh-windows-sandbox-setup.exe";
...
pub fn sandbox_bin_dir(codex_home: &Path) -> PathBuf {
    use sha2::{Digest, Sha256};
    let key = format!("{:x}", Sha256::digest(codex_home.to_string_lossy().to_lowercase().as_bytes()));
    std::env::var_os("ProgramData").map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(r"C:\ProgramData"))
        .join("DeepSeekHarnessNative").join(&key[..24]).join("bin")
}
...
fn run_setup_refresh_inner(
    ...
    assert_state_namespace(request.codex_home)?;
    ...
    let payload = ElevationPayload {
        ...
        otel: None,
        real_user: crate::token::current_user_sid_string()?,
        mode: SetupMode::Full,
        refresh_only: true,
    };
...
mod tests {
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let setup_exe = resources_dir.join("dsh-windows-sandbox-setup.exe");
...
/// Reject another product's persisted identity before changing any setup file.
pub fn assert_state_namespace(home: &Path) -> Result<()> {
    let (offline, online) = account_names(home);
    let owned = std::fs::read(home.join("dsh-native-owner.json")).ok()
        .and_then(|data| serde_json::from_slice::<serde_json::Value>(&data).ok())
        .is_some_and(|value| value["product"] == "deepseek-harness-rs" && value["version"] == 1);
    let marker = setup_marker_path(home);
    if marker.exists() {
        match serde_json::from_slice::<serde_json::Value>(&std::fs::read(marker)?) {
            Ok(value) => anyhow::ensure!(value["offline_username"] == offline && value["online_username"] == online,
                "STATE_OWNERSHIP_MISMATCH: refusing another application's sandbox state"),
            Err(_) if owned => {},
            Err(_) => {
                let users: serde_json::Value = serde_json::from_slice(&std::fs::read(sandbox_users_path(home))?)?;
                anyhow::ensure!(users["offline"]["username"] == offline && users["online"]["username"] == online,
                    "STATE_OWNERSHIP_MISMATCH: incomplete unowned sandbox state");
            }
        }
    }
    Ok(())
