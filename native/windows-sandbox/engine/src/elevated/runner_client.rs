...
pub(crate) fn spawn_runner_transport(
    ...
    let runner_cmdline = runner_exe
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "dsh-command-runner.exe".to_string());
    ...
    let cwd_w = to_wide(runner_exe.parent().ok_or_else(|| anyhow::anyhow!("runner has no trusted directory"))?);
    ...
    // A freshly provisioned non-interactive account has no profile directory.
    // Use the explicit execution environment instead of synthesizing one from
    // that absent profile, and do not inherit the broker's credentials.
    let env_block = Some(crate::process::make_env_block(&spawn_request.env));

    let active_record = codex_home.join("active-runner.json");
    std::fs::write(&active_record, br#"{"phase":"starting"}"#)?;
    ...
    if spawn_res == 0 {
        let err = unsafe { GetLastError() };
        crate::logging::log_note(&format!("runner launch failed: code={err}; executable={}; cwd={}",runner_exe.display(),cwd.display()),log_dir);
        unsafe {
            CloseHandle(h_pipe_in);
            CloseHandle(h_pipe_out);
        }
        let _ = std::fs::remove_file(&active_record);
        return Err(RunnerLogonError { code: err }.into());
    }
    let admission = (|| -> Result<()> {
        protect_runner_process(pi.hProcess)?;
        let created = process_created_at(pi.hProcess)?;
        std::fs::write(&active_record, serde_json::to_vec(&serde_json::json!({"phase":"running","pid":pi.dwProcessId,"created":created}))?)?;
        anyhow::ensure!(unsafe { windows_sys::Win32::System::Threading::ResumeThread(pi.hThread) } != u32::MAX, "cannot resume protected runner");
        Ok(())
    })();
    if let Err(error) = admission {
        unsafe { TerminateProcess(pi.hProcess, 125); CloseHandle(pi.hThread); CloseHandle(pi.hProcess); CloseHandle(h_pipe_in); CloseHandle(h_pipe_out); }
        return Err(error);
    }
    ...
    if let Err(err) = connect_result {
        unsafe {
            ...
            // that child instead of leaking a stray `dsh-command-runner.exe`.
            if pi.hProcess != 0 {
    ...
    let startup_result = (|| -> Result<()> {
        ...
        // runner instead of leaving a stray `dsh-command-runner.exe` behind.
...
fn process_created_at(process: HANDLE) -> Result<u64> {
    ...
    anyhow::ensure!(unsafe { windows_sys::Win32::System::Threading::GetProcessTimes(process, &mut created, &mut exited, &mut kernel, &mut user) } != 0, "cannot identify runner lifetime");
    Ok((u64::from(created.dwHighDateTime) << 32) | u64::from(created.dwLowDateTime))
}

fn protect_runner_process(process: HANDLE) -> Result<()> {
    use windows_sys::Win32::Security::{Authorization::{ConvertStringSecurityDescriptorToSecurityDescriptorW,SetSecurityInfo,SE_KERNEL_OBJECT},GetSecurityDescriptorDacl,DACL_SECURITY_INFORMATION,PROTECTED_DACL_SECURITY_INFORMATION};
    let principal = crate::token::current_user_sid_string()?;
    // OWNER RIGHTS suppresses implicit WRITE_DAC/WRITE_OWNER for a sandbox
    // account that owns the helper, while the broker retains its existing handle.
    let text = crate::to_wide(format!("D:P(D;;WDWO;;;OW)(A;;GA;;;SY)(A;;GA;;;{principal})"));
    let mut descriptor = std::ptr::null_mut();
    anyhow::ensure!(unsafe { ConvertStringSecurityDescriptorToSecurityDescriptorW(text.as_ptr(), 1, &mut descriptor, std::ptr::null_mut()) } != 0, "cannot construct runner protection");
    let mut present = 0; let mut defaulted = 0; let mut acl = std::ptr::null_mut();
    let ok = unsafe { GetSecurityDescriptorDacl(descriptor, &mut present, &mut acl, &mut defaulted) };
    let status = if ok != 0 && present != 0 { unsafe { SetSecurityInfo(process, SE_KERNEL_OBJECT, DACL_SECURITY_INFORMATION | PROTECTED_DACL_SECURITY_INFORMATION, std::ptr::null_mut(), std::ptr::null_mut(), acl, std::ptr::null_mut()) } } else { 87 };
    unsafe { windows_sys::Win32::Foundation::LocalFree(descriptor.cast()); }
    anyhow::ensure!(status == 0, "cannot protect native runner process: {status}");
    Ok(())
}
