...
pub fn main() -> Result<()> {
    anyhow::ensure!(!codex_windows_sandbox::setup_caller_is_restricted()?,"SETUP_DENIED: confined processes cannot initialize native identities");
    let ret = real_main();
    if let Err(e) = &ret {
...
mod tests {
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "DshSandboxOffline",
            "online_username": "DshSandboxOnline",
