...
pub const SANDBOX_USERS_GROUP: &str = "DshSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "DSH native sandbox internal group v1 (managed)";
...
pub fn provision_sandbox_users(
    ...
    let state = dunce::canonicalize(codex_home)?.to_string_lossy().to_lowercase();
    let identity = format!("{real_user_sid}\n{state}");
    let hash = format!("{:x}", Sha256::digest(identity.as_bytes()));
    let owner = format!("DSH native v1:{}", &hash[..32]);
    ...
    ensure_sandbox_user(offline_username, &offline_password, &owner, log)?;
    ensure_sandbox_user(online_username, &online_password, &owner, log)?;
    write_secrets(
        codex_home,
        offline_username,
        &offline_password,
...
pub fn ensure_sandbox_user(username: &str, password: &str, owner: &str, log: &mut dyn Write) -> Result<()> {
    ensure_local_user(username, password, owner, log)?;
    ensure_local_group_member(SANDBOX_USERS_GROUP, username)?;
    Ok(())
}

pub fn ensure_local_user(name: &str, password: &str, owner: &str, log: &mut dyn Write) -> Result<()> {
    let name_w = to_wide(OsStr::new(name));
    let pwd_w = to_wide(OsStr::new(password));
    let owner_w = to_wide(OsStr::new(&owner));
    unsafe {
        use windows_sys::Win32::NetworkManagement::NetManagement::{NetUserGetInfo, NetApiBufferFree};
        let mut existing = std::ptr::null_mut();
        let status = NetUserGetInfo(std::ptr::null(), name_w.as_ptr(), 1, &mut existing);
        if status == 0 {
            let record = &*(existing as *const USER_INFO_1);
            let recorded_owner = read_wide(record.usri1_comment);
            let owned = recorded_owner == owner;
            NetApiBufferFree(existing.cast());
            anyhow::ensure!(owned, "refusing to modify an account without this DSH ownership marker: {name}; expected={owner}; recorded={recorded_owner}");
        } else {
            anyhow::ensure!(status == 2221, "cannot verify account ownership for {name}: {status}");
        }
        let info = USER_INFO_1 {
            ...
            usri1_comment: owner_w.as_ptr() as *mut u16,
        ...
        if status != NERR_Success {
            anyhow::ensure!(status == 2224, "cannot create DSH account {name}: {status}");
...
pub fn ensure_local_group(name: &str, comment: &str, log: &mut dyn Write) -> Result<()> {
    ...
    unsafe {
        use windows_sys::Win32::NetworkManagement::NetManagement::{NetLocalGroupGetInfo, NetApiBufferFree};
        let mut existing = std::ptr::null_mut();
        let status = NetLocalGroupGetInfo(std::ptr::null(), name_w.as_ptr(), 1, &mut existing);
        if status == 0 {
            let record = &*(existing as *const LOCALGROUP_INFO_1);
            let owned = read_wide(record.lgrpi1_comment) == comment;
            NetApiBufferFree(existing.cast());
            anyhow::ensure!(owned, "refusing to adopt an unowned local group: {name}");
        } else {
            anyhow::ensure!(status == 2220 || status == 1376, "cannot verify group ownership: {status}");
        }
...
pub fn resolve_sid(name: &str) -> Result<Vec<u8>> {
    if name.starts_with("S-1-") {
        return sid_bytes_from_string(name);
    }
...
unsafe fn read_wide(value: *const u16) -> String {
    ...
    let mut len = 0;
    while len < 4096 && unsafe { *value.add(len) } != 0 { len += 1; }
