...
pub fn create_cwd_junction(requested_cwd: &Path, log_dir: Option<&Path>) -> Option<PathBuf> {
    ...
    let length = unsafe { windows_sys::Win32::System::SystemInformation::GetSystemDirectoryW(system.as_mut_ptr(), system.len() as u32) };
    if length == 0 || length as usize >= system.len() { return None; }
    let command = PathBuf::from(String::from_utf16_lossy(&system[..length as usize])).join("cmd.exe");
