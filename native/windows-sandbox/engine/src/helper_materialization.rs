...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "dsh-command-runner.exe",
        }
    }
...
mod tests {
    ...
    #[test]
    fn helper_bin_directory_is_shared_but_scoped_to_the_state_owner() {
        let codex_home = Path::new(r"C:\Users\example\.codex");

        let directory = helper_bin_dir(codex_home);
        assert!(!directory.starts_with(codex_home));
        assert_eq!(directory, helper_bin_dir(codex_home));
        assert_ne!(directory, helper_bin_dir(Path::new(r"C:\Users\another\.codex")));
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("dsh-command-runner.exe");
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let helper = resources_dir.join("dsh-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "dsh-command-runner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("codex.exe");
        let package_helper = package_resources_dir.join("dsh-command-runner.exe");
        let bin_helper = bin_resources_dir.join("dsh-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("codex.exe");
        let sibling_helper = release_dir.join("dsh-command-runner.exe");
        let resource_helper = resources_dir.join("dsh-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "dsh-command-runner-test-suffix.exe");
    }
}
