...
每条消息最多包含 16 个普通文件，普通文件不另设单文件字节上限；图片默认不设本地字节上限，部署配置中的 `maxImageBytes` 和 `maxMessageImageBytes` 为 `0` 时表示不限。上传采用 JSON 传输，单个 HTTP 请求上限为 300 MiB，包含 Base64 编码和消息正文。图像数量、解码像素和模型请求容量按各自配置校验。普通文件验证与落盘使用固定大小缓冲区，不同时保留整批解码副本。普通文件可以为空。文件名不得包含路径分隔符、控制字符或系统保留名称。

目录按会话和内容摘要隔离，同名不同内容具有不同路径。保存采用不覆盖的原子发布；重试复用相同内容，遇到内容冲突返回错误。提交失败保留输入草稿，切换会话不会串用附件。发送后的文件卡片使用准确路径打开，原始消息内容保持完整。

## Agent Teams

在“设置 → 团队”开启团队功能并选择成员上限，保存后重启应用；会话顶部的“团队”入口也可打开这些设置。配置格式：
...
会话标题栏中的“团队”面板显示成员和共享任务，可打开成员会话；功能关闭时仍显示入口，并提供启用设置。点击“填写团队任务”会将团队任务草稿追加到输入框，用户编辑并发送后才开始执行。主会话永久删除时，团队成员遵循同一子智能体清理规则。
