...
Computer Use 工作台会先列出当前对话已有的浏览器会话，再连接模型使用的命名会话；可通过“浏览器会话”选择其他页面，或刷新会话列表。查看已有会话不会重新导航到侧栏曾保存的地址。切换查看对象保留原浏览器，关闭控制页仅关闭当前选定目标及会话。

“允许一次”只执行当前动作；“始终允许”在当前 Host 运行期间记住当前对话、适配器和控制会话的授权。同一浏览器的后续导航和输入可复用该授权，其他浏览器会话、本机桌面与远程桌面分别授权。新的禁止策略、取消信号和不可记忆的请求优先于已有授权。人工接管与工具审批分别管理；人工接管后仍须在控制面板交还智能体。

浏览器导航接受 HTTP、HTTPS 和 `about:blank`。本地 HTML 应通过已就绪的 localhost HTTP 服务访问；`file:` 地址在弹出工具审批及启动浏览器之前即被拒绝，不会留下空白浏览器进程。
...
设置中的“Computer Use 与远程设备”查询当前 UU 账号、列出设备并保存绑定。客户端未安装时提供安装入口，未登录时提示在 UU 客户端登录；账号切换后旧绑定不再用于连接。

`native-desktop` 使用 Windows 原生截图与键鼠接口，`uu-desktop` 使用已安装的 UU SDK 连接已绑定的远程设备；两者在独立 Rust 进程中运行，共用工作台和控制权管理。UU 路径直接从 Rust 自建的渲染窗口获取画面，不依赖官方控制窗口。功能边界与人工接管规则见 [Computer Use](sidebar-workbench-suite.md#computer-use)。
