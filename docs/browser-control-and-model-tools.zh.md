...
独立浏览器会话的工具调用与截图观察循环已接入。设置中的“Computer Use 与远程设备”使用 UU 官方 CLI 查询当前账号、列出设备并保存绑定，连接与断开始终指定当前账号下的设备 ID。客户端未安装时提供官方安装入口，未登录时提示在 UU 客户端登录；账号切换后旧绑定不再用于连接。

UU 的远程桌面在官方客户端中显示。公开 CLI 尚未提供桌面截图和键鼠注入接口，因此设备绑定与连接管理不等同于模型能够操作远端桌面的控件。
