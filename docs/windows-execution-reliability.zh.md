...
执行环境错误与 Agent 生命周期错误分别处理，避免一次工具或插件异常阻塞后续对话。

| 边界 | 行为 | 回归覆盖 |
| --- | --- | --- |
| 驱动任务 | panic 关闭已打开的回合，释放 Running 状态与活动等待器 | 前置钩子 panic 后能够提交下一条消息 |
| 发起者作用域 | 正常返回、panic、Future 被丢弃均释放运行计数 | 作用域退出后注册表可完成关闭 |
| 取消信号 | 注册等待器后检查状态，取消状态对后来的等待者仍有效 | 多等待者与取消并发 |
| 工具调度 | 前置准备可取消且保留执行上下文；初始填充不在提交中途被丢弃 | 非协作前置/后置钩子、结果配对、finalizer 恰好一次 |
| 并发配置 | 驱动读取 agentLoop 的 maxParallelToolCalls；零容量直接报错 | 容量为 1 时第二个并行工具不会提前派发 |
| 会话发布 | 写入由发布守卫串行化；前置钩子可读取已提交历史；重入写入返回错误 | 重入、条件回调 panic、前置读取与后续发布 |
| 同步监听器 | session/event 与 internal/dispatch 监听器必须单次轮询完成；挂起监听器被拒绝并记录诊断 | 单线程运行时中的等待监听器不会阻塞其余通知 |
| 沙箱准备缓存 | 失败的共享准备允许重试；执行中及成功的准备继续复用 | 失败、成功、未完成三种缓存状态 |

需要异步 I/O 的会话观察器应自行派发后台任务，并立即返回；需要否决或等待的生命周期钩子应使用对应的异步接口。同步发布接口不会等待异步观察器完成。

上述边界不提供新的 Windows 隔离身份，也不改变命名管道权限。独立账号后端、Node 管道子进程与跨用户权限矩阵仍须完成通用后端验收后才能部署。
