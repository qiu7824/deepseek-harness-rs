...
登录、重新连接和目录刷新同步当前账号可用模型；缓存按账号隔离，显示开关、别名和明确手工覆盖独立保存。目录刷新使用版本校验，避免覆盖同时进行的用户编辑。

模型目录可返回上下文容量、最大输出、输入模态、推理档位及默认值。提供商明确返回的能力优先；已知具体型号使用保守映射，未知型号保留自动配置。`reasoning: true` 不足以推导出 low、high 或 xhigh 等档位。

显示开关控制选择器候选项，保留已有会话的模型引用和解析能力。更改提供商配置不会删除会话、工作区或记忆。

## 免费版验证

免费版预置通过完整发布验证的模型。发布流程将官方模型 ID／协议表与免费价格表关联，再对每个候选执行匿名流式推理、工具调用、工具结果续接及正式二进制运行检查。一个候选限流或失败不影响其他已通过候选；没有任何通过者时不生成免费版。

包内 `free-model-verification.json` 的成功证据不超过 24 小时，并与包内运行时 SHA256 一致。应用内重新检测仅证明当前连接和协议可用，不替代发布时的二进制验证。未确认免费价格的候选保持待验证状态，不发送推理请求。
